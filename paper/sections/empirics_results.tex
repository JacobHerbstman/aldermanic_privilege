...
\end{equation}
%
Here $Y_{isy}$ is FAR or DUPAC for project $i$ on boundary segment $s$, built in calendar year $y$.
The running variable $d_{is}$ is negative on the less-stringent side and positive on the more-stringent side.
The set $\mathcal{K}$ contains ten 100ft bands from $-500$ to 500ft.
The band from $-100$ to 0ft is omitted, so $\kappa_{[0,100)}$ compares projects immediately across the boundary.
...
As in the density analysis, $\mathcal{K}$ contains ten 100ft distance bands and the band from $-100$ to 0ft is omitted.
