...
The border design compares nearby parcels on opposite sides of ward boundaries. A complementary test is whether development activity changes when a fixed location is reassigned to a different alderman. I therefore exploit the 2015 ward redistricting as a natural experiment, which reassigned census blocks to new aldermen for reasons unrelated to block-level development activity. This lets me compare the same census block before and after its alderman changes, so block fixed effects absorb any time-invariant differences across locations.
...
Several features of the design support this assumption. Redistricting is driven primarily by population equalization and political bargaining over ward composition, rather than by parcel-level development activity on specific blocks. To tighten the counterfactual, I restrict the control group to blocks that remained in the \textit{origin} ward, so treated and control blocks shared the same pre-treatment regulatory environment. I also drop non-switching blocks in origin wards where the alderman changed between 2014 and 2015. Those would be contaminated controls: they did not redistrict into a new ward, but they still experienced an alderman change through election. Figure~\ref{fig:identification_example} illustrates the design for one ward pair, showing how blocks are classified into treatment and control groups based on the change in alderman stringency. Figure~\ref{fig:treatment_control_maps} shows the citywide distribution of treated and control blocks for the 2015 redistricting.
...
The outcome is the count of high-discretion permits issued on each census block in each year from 2010 through 2020. Because permits are counts that aggregate naturally to a balanced block-year panel, with every block observed in every year (including zeros), I can include block fixed effects and identify entirely off within-block variation over time. I estimate the event study using Poisson pseudo-maximum likelihood (PPML), with the log conditional mean given by:
%
\begin{equation}
...
\end{equation}
%
where $Y_{bt}$ is the count of high-discretion permits issued on block $b$ in year $t$, $\alpha_b$ is a block fixed effect, and $\lambda_{gt}$ is a border-pair-by-year fixed effect. In the 2015-cohort design, the treatment variable $\Delta\text{Stringency}_b$ is the destination alderman's baseline strictness score minus the origin alderman's baseline strictness score, where origin and destination aldermen are assigned using the 2014 pre-redistricting ward-to-alderman mapping.

The block fixed effects absorb all time-invariant characteristics of each block, so identification comes from within-block changes over time: the same physical block before and after its alderman changes. The border-pair-by-year fixed effects absorb local shocks that affect all blocks near a given ward boundary in a given year, so the comparison is between blocks near the same boundary that experienced different changes in aldermanic representation. The boundary neighborhoods are defined using the cohort's pre-redistricting ward map: switched blocks use the fixed origin-destination boundary created by redistricting, and non-switched controls use their nearest incident pre-redistricting boundary. The sample uses the 2015 redistricting cohort from 2010 through 2020, a 1000ft bandwidth around those boundaries, and uniform weights. I stop in 2020 to keep a symmetric window around the 2015 implementation and avoid mixing the event-study block panel with the later census-block vintage. I use a wider window here than in Section~\ref{sec:empirical_results_density} because this design does not identify from a discontinuity exactly at the boundary. It compares treated and control blocks within the same boundary neighborhoods over time, so the extra width preserves local support and power without changing the fixed-effects comparison. Blocks with zero permits in some years remain in the panel and are informative under PPML. Blocks with zero permits in every sample year contribute no within-block variation once block fixed effects are included, so the estimator drops them mechanically. Standard errors are clustered at the census block level.

\subsection{Results}

Figure~\ref{fig:permit_event_study} presents the main result. The pre-period coefficients are statistically indistinguishable from zero, and the joint pre-trend test does not reject. After redistricting, the coefficients turn negative and remain negative through most of the post-period. The pattern is consistent with reduced permitting when the regulatory gatekeeper changes.
...
\begin{figure}[htbp]
    ...
    \figurenotes{Block-year PPML event study for calendar years 2010--2020. Outcome is the count of high-discretion permits issued per block per year. 2015 redistricting cohort; sample restricted to blocks within 1000ft of ward boundaries with uniform weights. Continuous treatment: effect of a 1 SD increase in alderman stringency. Block and border-pair $\times$ year fixed effects. Year $-1$ omitted as reference. 95\% confidence intervals based on standard errors clustered at the block level. Pooled post-period estimate: $-0.077$ (SE $= 0.037$). Joint pre-trend test $p = 0.881$.}
...
Table~\ref{tab:did_permit_2015_current} collapses the post-period into a single pooled DID coefficient. The pooled PPML estimate is $-0.077$ (SE $= 0.037$), which is about a 7.4\% decline for a one standard deviation increase in alderman stringency. This is a direct measure of how aldermanic discretion affects project flow: more stringent aldermen do not simply alter the scale of completed buildings, they reduce the number of projects that require their engagement.
...
\begin{table}[htbp]
    ...
    \footnotesize
    \textit{Notes:} Block-year PPML regression using the 2015 implementation cohort, restricted to census blocks within 1000ft of ward boundaries and calendar years 2010--2020. Outcome is the number of issued high-discretion permits. Uniform weighting. Standard errors clustered at the block level. * $p<0.10$, ** $p<0.05$, *** $p<0.01$.
\end{table}
