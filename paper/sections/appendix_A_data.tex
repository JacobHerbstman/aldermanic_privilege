...
Rental CTA distances use stations open by the listing month.
...
Treatment intensity is measured using the difference between the destination and origin aldermen's baseline strictness scores.
...
The change in strictness for block $i$ is $\Delta\text{Strictness}_i = \text{Strictness}_{\text{dest}} - \text{Strictness}_{\text{origin}}$.
Binary specifications label blocks as ``moved to stricter'' if $\Delta\text{Strictness}_i > 0$ and ``moved to more lenient'' if $\Delta\text{Strictness}_i < 0$.
...
The full sample contains 12,451 new residential buildings with an average of 6.45 dwelling units and 2.28 stories.
The average Floor Area Ratio (FAR) is 1.20 and the average Dwelling Units Per Acre (DUPAC) is 27.91.
Multifamily buildings are much denser, with an average of 32.85 units, 2.76 stories, FAR of 2.30, and DUPAC of 74.75.
Median distance to the nearest ward boundary is about 930ft in the full sample and 840ft in the multifamily sample.
...
\paragraph{Geocoding.}
Each sale is geocoded by joining the property identification number (PIN) to the Cook County parcel universe, which contains latitude and longitude coordinates.
Sales that cannot be matched on the full 14-digit PIN are matched on the first 10 digits (PIN10).
...
Table~\ref{tab:rental_data_validation_growth} compares real citywide listed-rent levels in the cleaned panel to Zillow's Chicago Observed Rent Index, the closest external asking-rent benchmark.
...
\begin{table}[H]
    \caption{Listed-Rent Validation: Real Citywide Rent Levels and Growth}
