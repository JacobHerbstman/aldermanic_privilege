...
I exclude permits that follow expedited review processes: (1) permits in the ``Easy Permit Process''; (2) permits in the ``Express Permit Program''; (3) signs permits; and (4) scaffolding permits. These exclusions target permit types that bypass standard aldermanic review and thus do not reflect aldermanic influence on development.
...
Table~\ref{tab:stage1_residualization} reports the Stage 1 residualization regression. The dependent variable is log mean processing time at the ward-month level, weighted by the number of permits. The coefficients reveal systematic relationships between ward characteristics and permit processing times. Wards closer to the CBD and with higher median incomes have shorter processing times, while wards with greater lakefront exposure, more CTA stations, and higher homeownership rates have longer processing times. These patterns are absorbed in the residualization, ensuring that the alderman fixed effects capture only the component of processing time attributable to the alderman rather than ward characteristics.
...
\begin{table}[htbp]
    ...
    \footnotesize
    \textit{Notes:} Dependent variable is log mean permit processing time at the ward-month level. Observations weighted by the number of high-discretion permits. Community area fixed effects (77 areas) included but not shown. Month fixed effects absorb city-wide temporal variation in processing times.
\end{table}
