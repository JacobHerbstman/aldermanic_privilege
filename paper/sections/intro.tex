...
Density not only makes housing more affordable, but there are well-documented positive agglomeration spillovers from density, 
...
This privilege is so strong, in fact, that aldermen have been referred to as ``little mayors" ruling their Wards as fiefdoms \citep{einhorn_property_1991}.
...
Department of Housing and Urban Development alleging that the power has been used to ensure affordable housing is not built in certain affluent wards \citep{chase_chicago_2023}.
In response to these allegations, Mayor Lightfoot signed an executive order attempting to limit the power of aldermanic privilege as part of a larger ethics reform package \citep{mayors_press_office_mayor_2019},
but in practice not much has changed in the years since \citep{cherone_developer_2025}. 
...
To examine whether these supply restrictions pass through into housing affordability, I exploit the 2015 and 2023 ward redistrictings as natural experiments. When ward boundaries are redrawn for population balancing, some census blocks are reassigned to new aldermen while nearby blocks remain under the same representation. This creates a difference-in-differences comparison where the identifying assumption is that reassigned and non-reassigned blocks would have followed parallel trends absent the change in aldermanic representation. Crucially, this design circumvents the selection problem inherent in the cross-sectional analysis: developers cannot anticipate or sort in response to redistricting that has not yet occurred.

I find that census blocks reassigned to stricter aldermen experience approximately 4\% higher home sale prices and 3–4\% higher rents relative to nearby blocks that did not switch aldermen. These effects emerge gradually over the post-redistricting period, consistent with a supply restriction channel where reduced new construction slowly tightens the housing market. The price and rent results complement the density findings: stricter aldermen approve less housing (Section \ref{sec:empirical_results_density}), which restricts supply and raises equilibrium housing costs (Section \ref{sec:empirical_results_rent_home_vals}).

The rest of the paper is organized as follows: Section \ref{sec:data} describes the data and institutional background of aldermanic privilege in Chicago.
Section \ref{sec:model} presents a toy model of fragmented land-use authority and derives testable predictions.
Section \ref{sec:empirical_results_density} presents the empirical results on development density at ward boundaries, and Section \ref{sec:empirical_results_rent_home_vals} presents the results on home prices and rents following ward redistricting.
Section \ref{sec:discussion} discusses the policy implications of these findings, and Section \ref{sec:conclusion} concludes.