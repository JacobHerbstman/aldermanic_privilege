...
\begin{figure}[htbp]
    ...
    % Log DUPAC bandwidth sensitivity
    \begin{minipage}{0.48\textwidth}
        \centering
    ...
    \end{minipage}
    \caption{Bandwidth Sensitivity: Log(FAR) (top row) and Log(DUPAC) (bottom row) at 250 and 1,000ft bandwidths}
    \label{fig:rd_bandwidth_robust}
\end{figure}

\subsubsection{Additional Robustness Checks}

Additional robustness checks are presented in Appendix~\ref{sec:appendix_density}. These include: (1) a ``donut RD'' specification that excludes parcels within 25 or 50 feet of the ward boundary to address concerns about observations precisely at the boundary (Figure~\ref{fig:rd_donut}); (2) alternative functional forms using quadratic and cubic polynomial specifications (Figure~\ref{fig:rd_polynomial}); and (3) placebo boundary tests at artificial cutoffs 500 feet inside each ward (Figure~\ref{fig:rd_placebo}). All robustness checks support the main findings.
