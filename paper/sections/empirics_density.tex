...
In Figure~\ref{fig:rd} I present regression discontinuity plots for my two primary measures of density, DUPAC and FAR.
...
The variable of interest is $\text{Strictness}_{w}$, which denotes the standardized bureaucratic friction score for the alderman of ward $w$. To control for spatial trends, I include $|D_{i}|$, the absolute distance of the parcel to the ward boundary. The vector $\mathbf{X}_{w}$ includes time-varying ward-level controls, including median household income, racial composition, and education levels.
...
In addition, following \cite{black_better_1999,bayer_unified_2007,kulka_how_2022}, I include border-pair fixed effects $\mu_{bt}$, which are interacted with zoning code fixed effects to ensure comparisons are being made within border-pairs and within zoning classifications.
