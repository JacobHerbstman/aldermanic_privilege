...
I also remove the demographic controls from my regressions and run the same specifications, but with demographics as the outcome variables, in Table~\ref{tab:fe_balance_250} in the Appendix to see if aldermanic strictness is correlated with changes in ward demographics that could be driving my results.
I find null results across the share of residents that are white, average household income, and the share of residents with a bachelor's degree or higher, suggesting that the differences in density at the ward borders are being driven by aldermanic behavior and not purely by demographic sorting.
...
The spatial regression discontinuity results presented in Section 4.2 are subject to several potential concerns common to RD designs. In this section, I present bandwidth sensitivity in the main text and provide additional robustness checks in Appendix~\ref{sec:appendixa} that address concerns about observations precisely at the boundary, functional form assumptions, and the validity of the discontinuity itself.

\subsubsection{Bandwidth Sensitivity}

A key choice in any RD design is the bandwidth---the window around the cutoff within which observations are used for estimation. My main specification uses a 500-foot bandwidth, but results should be robust to reasonable alternatives. Figure~\ref{fig:rd_bandwidth_robust} presents RD plots for both log(DUPAC) and log(FAR) at bandwidths of 250, 500, and 1,000 feet. The discontinuity remains visible and statistically significant across all bandwidth choices for both density measures, though precision naturally decreases with narrower bandwidths due to smaller sample sizes. The point estimates are relatively stable, suggesting the results are not driven by a particular bandwidth choice.
