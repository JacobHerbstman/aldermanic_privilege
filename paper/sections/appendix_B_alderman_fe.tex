...
Raw alderman fixed effects are shrunk toward the grand mean using an empirical Bayes procedure: $\hat{\alpha}_a^{\text{EB}} = B_a \hat{\alpha}_a$, where $B_a = \hat{\omega}^2/(\hat{\omega}^2 + \hat{\sigma}_a^2)$ is the shrinkage factor and $\hat{\omega}^2$ is the estimated between-alderman variance.

\paragraph{Index Standardization.}
The final stringency index, $S_a$, is standardized to have mean zero and unit standard deviation across aldermen.
