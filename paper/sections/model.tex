...
\begin{itemize}
    \item \textbf{$A(H)$ - Citywide Agglomeration Benefits:} This function represents the total value created by housing density, including increased property tax revenue, labor market pooling, and aggregate affordability. I assume $A'(H) > 0$ and $A''(H) < 0$ (diminishing marginal returns to density). Importantly, these benefits are \textbf{diffuse}; they are shared by the city as a whole.
    
    \item \textbf{$C(h_g)$ - Local Congestion Costs:} This represents the ``NIMBY'' costs associated with density—traffic, loss of light, or strain on local amenities. I assume these costs are concentrated entirely within the ward where construction occurs, with $C'(h_g) > 0$ and $C''(h_g) > 0$ (convex costs).
...
By differentiating with respect to our parameters of interest, one can see that housing supply is decreasing in the bureaucratic friction parameter $\kappa$ and increasing in the local capture parameter $\lambda$:
...
While local control is typically viewed as a preference parameter ($\lambda$), heterogeneity in bureaucratic frictions ($\kappa$) can generate similar outcomes, motivating the empirical strategy outlined in Section~\ref{sec:empirical_results_density} where I directly create a measure of bureaucratic frictions induced by each Alderman.
