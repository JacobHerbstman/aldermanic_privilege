...
\begin{equation}
y_{bpet} = \alpha_{pe} + \lambda_{et} + \sum_{\tau=-5, \tau \neq -1}^{5} \beta_\tau \left( \Delta\text{Strictness}_b \times \mathbf{1}[t = \tau] \right) + \varepsilon_{bpet}
\end{equation}

\noindent where $y_{bpet}$ is the outcome (log mean sale price or log mean rent) for census block $b$ along border-pair $p$ in event cohort $e$ (2015 or 2023 remap) at time $t$. $\Delta\text{Strictness}_b$ is the change in strictness for block $b$, defined as the strictness score of the destination alderman minus the strictness score of the origin alderman. In the continuous treatment specification, this enters as a continuous variable; I also present binary specifications below. $\alpha_{pe}$ are cohort-by-ward-pair fixed effects, which ensure comparisons are made between treated and control blocks along the same ward boundary within each redistricting event. $\lambda_{et}$ are event-by-year fixed effects, allowing for different time trends across the two redistricting episodes.
...
\begin{figure}[htbp]
    ...
    \figurenotes{Panel A illustrates treatment assignment for ward pair 13--23 during the 2015 redistricting: ward assignments before (left), after (center), and treatment status based on the change in alderman strictness (right). Panels B and C show the city-wide distribution of treated and control blocks for 2015 and 2023. Sample restricted to blocks within 1,000 feet of ward boundaries. Controls exclude blocks in wards with alderman turnover via election.}
