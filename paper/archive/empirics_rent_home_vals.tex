...
The key fixed effects ensure that comparisons are made within localized boundary segments. $\alpha_{gs}$ is a segment-by-origin-side fixed effect, ensuring that treated and control observations are compared on the same boundary segment and origin side. $\lambda_{gt}$ is a segment-by-year fixed effect, which absorbs time-varying shocks at the segment level rather than pooling along full ward-pair boundaries. For stacked specifications, these fixed effects are cohort-indexed. Standard errors use two-way clustering by block and segment to account for serial correlation within blocks and shared segment-level shocks.

The coefficients $\beta_\tau$ trace out the effect of aldermanic strictness on housing costs in event time, with $\tau = -1$ (the year before redistricting takes effect) as the omitted reference period. The identifying assumption implies $\beta_\tau = 0$ for $\tau < 0$: prior to redistricting, there should be no systematic relationship between future changes in alderman strictness and housing cost trends within the same segment-side cell.

I restrict the sample to observations within 300m of ward boundaries and apply kernel weights that give more weight to observations closer to the boundary, where treated and control units are most comparable. In the continuous treatment specification, $\Delta\text{Strictness}$ enters as a continuous variable; I also present binary specifications that allow for separate effects by treatment direction (moved to stricter versus moved to more lenient).
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.98\textwidth,height=0.76\textheight,keepaspectratio]{../tasks/event_study_sales_diagnostics/output/ward_pair_vertical_13-23_300m.pdf}
    \caption{Identification Example: Ward Pair 13--23 (2015 Redistricting)}
    \figurenotes{Top panel shows ward assignments before redistricting (2014 boundaries), middle panel shows assignments after redistricting (2015 boundaries), and bottom panel shows treatment status based on the change in alderman strictness. Blocks switching from Ward 13 to Ward 23 are compared to controls that remained in Ward 13 (the origin ward), and vice versa. Sample restricted to blocks within 300m of ward boundaries.}
...
\paragraph{Data and Sample.} For rental listings, I use scraped listing data from 2014--2025, which allows me to stack both the 2015 and 2023 redistricting events. This stacked design maximizes statistical power by pooling two natural experiments. The rental sample includes listings within 300m of ward boundaries. For home sales, I use transaction data from the Cook County Recorder of Deeds for the 2015 redistricting cohort, with a sample window from 2010--2020.\footnote{I focus on the 2015 redistricting for home sales because the 2023 redistricting provides limited post-period data. Appendix~\ref{sec:appendix_sales_rental} presents additional robustness checks.}
...
\input{../tasks/_deprecated/summary_stats_event_study/output/summary_stats_event_study.tex}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../tasks/run_event_study_rental_disaggregate/output/event_study_disaggregate_yearly_stacked_continuous_300m_short.pdf}
        \caption*{Panel A: Continuous Treatment}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../tasks/run_event_study_rental_disaggregate/output/event_study_disaggregate_yearly_stacked_binary_direction_300m_short.pdf}
        \caption*{Panel B: Binary Treatment}
    \end{minipage}
    \caption{Event Study: Effect of Alderman Strictness on Rents}
    \figurenotes{Panel A: continuous treatment (effect of 1 SD increase in strictness). Panel B: binary treatment (separate effects for blocks moving to stricter vs.\ more lenient aldermen). Stacked estimator combining 2015 and 2023 redistricting cohorts. Cohort-by-segment-by-side and cohort-by-segment-by-year fixed effects. Kernel weights with 300m bandwidth. Includes hedonic controls for building type, unit area, bedrooms, and bathrooms. Year $-1$ omitted as reference. Post-period truncated at $t=2$ where both cohorts contribute observations. 95\% confidence intervals use two-way clustering by census block and segment.}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../tasks/run_event_study_sales_disaggregate/output/event_study_disaggregate_yearly_cohort_2015_continuous_uniform_300m_amenity_full_geo_wardpair_clust_block.pdf}
        \caption*{Panel A: Continuous Treatment}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../tasks/run_event_study_sales_disaggregate/output/event_study_combined_disaggregate_yearly_cohort_2015_continuous_split_uniform_300m_amenity_full_geo_wardpair_clust_block.pdf}
        \caption*{Panel B: Binary Treatment}
    \end{minipage}
    \caption{Event Study: Effect of Alderman Strictness on Home Sale Prices}
    \figurenotes{Panel A: continuous treatment (effect of 1 SD increase in strictness). Panel B: continuous split by treatment direction. 2015 redistricting cohort. Border-pair-by-side and border-pair-by-year fixed effects. Uniform weights with 300m bandwidth. Includes hedonic controls for building area, land area, building age, bedrooms, bathrooms, and garage. Year $-1$ omitted as reference. 95\% confidence intervals use clustering by census block.}
...
\input{../tasks/_deprecated/combine_did_tables/output/did_table_simple.tex}
