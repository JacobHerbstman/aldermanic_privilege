...
The event-study outcomes include applications that were issued or later expired, while applications marked cancelled, revoked, or suspended are excluded.
...
\paragraph{Electoral-Turnover Exclusion.}
I retain a block only if the 2014 aldermen serving both its origin and destination wards remained in their respective offices in 2015.
Without this restriction, the measured change in stringency could reflect the 2015 election rather than redistricting, including for blocks whose origin ward elected a new alderman.

\paragraph{Analysis Sample.}
Permit applications are aggregated to block-year panels using application year.
The event study uses the 2015 implementation cohort with relative years $-5$ through $5$.
The 500ft local-support restriction is based on the distance from each census block polygon's centroid to the cohort's pre-redistricting boundary geometry.
