...
\paragraph{Aldermanic Turnover.}
For each block, I measure the change in stringency using the 2014 incumbents in its origin and destination wards.
The main sample retains blocks only when both incumbents remained in office when the new map took effect.
The assigned change therefore matches the change the block actually experienced without defining treatment using the outcome of the 2015 election.
Appendix~\ref{sec:appendix_permit_assigned} includes blocks from wards with electoral turnover and interprets the resulting estimate as the effect of assigned stringency.
...
The regressions allow each block's total permit volume in years $-5$ through $-1$ to have a different coefficient in each calendar year.
They also include one interaction between the post-redistricting period and an indicator for having no permits in years $-5$ through $-1$.
...
The same project can appear more than once when a parcel is divided, receives a new PIN, or appears in both Assessor files.
I link these records so that each construction project enters the sample once.
I then compare the sample with every issued residential new-construction permit.
This comparison adds completed projects missing from the Assessor records and identifies permits for which units, building area, and lot area cannot be measured consistently.
...
These restrictions exclude vacant parcels, data errors, and buildings completed outside 2006--2022.
...
Assessor records sometimes report different construction years for the same project.
When they do, I use permit and completion dates to choose the year that best matches when the building was completed.
Properties with multiple assessment cards can also repeat the same whole-building characteristics.
I combine cards when they clearly describe one building and do not count later PINs that represent the same construction a second time.
For 52 projects citywide, including 26 within 500ft of a ward boundary, these rules do not fully resolve the record.
I record the decision for each project in a hard-coded spreadsheet included with the replication files.

\paragraph{Geocoding.}
I first locate each original PIN using the Historical Parcel Universe for the construction year.
When those records have no matching parcel, I use the current location of the same PIN or a verified historical address.
I keep one location for each project so that old and new PINs for the same site are not counted separately.

\paragraph{Multifamily Classification.}
I primarily use the Assessor's property class to distinguish multifamily projects from collections of single-family buildings.
When the reported unit count does not plausibly describe the whole building, I use permit descriptions and other property records to recover it.
A small hard-coded spreadsheet records the remaining case-by-case classifications.
...
For each project, I compute two density measures: Floor Area Ratio (FAR), defined as total building area divided by lot area; and Dwelling Units Per Acre (DUPAC), defined as the number of dwelling units divided by lot acreage.

\paragraph{Ward and Boundary Assignment.}
Each project is assigned to a ward based on its location and the ward boundaries in effect at the time of construction.
...
Table~\ref{tab:summary_stats} presents summary statistics for the projects within 500ft used in both the main FAR and DUPAC regressions.
