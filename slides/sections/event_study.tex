...
\begin{frame}
    ...
    \textbf{Specification}:
    \[
    \ln(P_{it}) = \sum_{\tau \neq -1} \beta_\tau \cdot \mathbf{1}[t = \tau] \cdot \Delta\text{Strictness}_i + X_{it}'\gamma + \alpha_{gs} + \lambda_{gt} + \varepsilon_{it}
    \]
    ...
    \begin{itemize}
        ...
        \item $\alpha_{gs}$: Segment $\times$ origin-side FE
        \item $\lambda_{gt}$: Segment $\times$ year FE
    \end{itemize}
    
    \vspace{0.5em}
    \textbf{Key assumption}: Parallel trends within each segment $\times$ side cell
    
    \vspace{0.3em}
    \small
    Sample: Within 1,000 ft of boundaries; triangular weights $w_i = 1 - \frac{d_i}{1000}$; two-way clustered SEs (block + segment)
\end{frame}
...
\begin{frame}[label=rent-main]
    ...
    \begin{column}{0.48\textwidth}
        ...
        \begin{figure}
            \includegraphics[width=\textwidth,height=0.55\textheight,keepaspectratio]{../tasks/run_event_study_rental_disaggregate/output/event_study_disaggregate_yearly_stacked_continuous_triangular_1000ft_mf_short.pdf}
        \end{figure}
    \end{column}
    \begin{column}{0.48\textwidth}
        \centering
        ...
        \begin{figure}
            \includegraphics[width=\textwidth,height=0.55\textheight,keepaspectratio]{../tasks/run_event_study_rental_disaggregate/output/event_study_disaggregate_yearly_stacked_continuous_split_triangular_1000ft_mf_short.pdf}
        \end{figure}
    \end{column}
    \end{columns}
...
\begin{frame}[label=price-main]
    ...
    \vfill\hfill
    \hyperlink{sales-bw500}{\beamergotobutton{500ft}}
    \hspace{0.2em}
...
\begin{frame}
    ...
    \vspace{0.5em}
    \small
    Segment $\times$ side FE and segment $\times$ year FE. Triangular kernel, 1000 ft bandwidth, two-way clustering (block + segment).
