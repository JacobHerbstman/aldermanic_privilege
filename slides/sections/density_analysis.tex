...
\begin{frame}
    ...
    \toprule
     & All New & Multifamily \\
    \midrule
    Average FAR & 1.20 & 2.29 \\
    Average units & 6.43 & 32.79 \\
    Average stories & 2.28 & 2.76 \\
    Average DUPAC & 27.84 & 74.48 \\
    Median border distance & 930 ft & 832 ft \\
    N & 12,476 & 2,132 \\
    \bottomrule
    \end{tabular}
    \end{center}
...
\begin{frame}[label=identification-strategy]
    ...
    \begin{itemize}
        ...
        \item \textbf{Regressions}: Binary boundary comparison and continuous score-difference specification
    \end{itemize}
\end{frame}
...
\begin{frame}[label=visual-rd-main]
    ...
    \scriptsize
    \textit{Notes}: All-construction and multifamily samples, 500 ft bandwidth, zoning-group + segment + year FE, ward-pair clustered SEs.
...
\begin{frame}[label=density-reg-main,shrink=8]
    ...
    \small
    \textbf{Specification}: 
    $\ln(Y_i) = \beta T_i + \eta \overline{S}_{ww'} + \alpha_{\text{zone group}} + \alpha_{\text{segment}} + \alpha_{\text{year}} + \varepsilon_i$\\
    \scriptsize $T_i$ is the more-stringent-side indicator in Panel A and one-half of the own-minus-neighbor score difference in Panel B.
    \small
    ...
    \vspace{0.1em}
    \textbf{Interpretation}: Multifamily FAR and DUPAC are about 24\% and 28\% lower on the more stringent side.
