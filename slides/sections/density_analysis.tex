...
\begin{frame}
    ...
    \vspace{0.3em}
    \textbf{Identifying variation}: Within the same boundary segment, controlling for zoning group and citywide year fixed effects, does the more stringent side have less dense new multifamily construction?
...
\begin{frame}[label=visual-rd-main]
    ...
    \begin{column}{0.48\textwidth}
        ...
        \textbf{Log(FAR)}
        \begin{figure}
            \includegraphics[width=\textwidth,height=0.55\textheight,keepaspectratio]{../tasks/spatial_rd_fe/output/rd_fe_plot_log_density_far_bw500_multifamily_zonegroup_segment_year_additive_clust_segment.pdf}
        \end{figure}
    \end{column}
    \begin{column}{0.48\textwidth}
        \centering
        \textbf{Log(DUPAC)}
        \begin{figure}
            \includegraphics[width=\textwidth,height=0.55\textheight,keepaspectratio]{../tasks/spatial_rd_fe/output/rd_fe_plot_log_density_dupac_bw500_multifamily_zonegroup_segment_year_additive_clust_segment.pdf}
        \end{figure}
    \end{column}
    \end{columns}
    
    \scriptsize
    \textit{Notes}: Multifamily sample, 500 ft bandwidth, zoning-group + segment + year FE, segment-clustered SEs.
...
\begin{frame}[label=density-reg-main]
    ...
    \begin{center}
    \input{../tasks/border_pair_FE_regressions/output/fe_table_bw500_multifamily_zonegroup_segment_year_additive_clust_segment.tex}
    \end{center}
    
    \vspace{0.1em}
    \textbf{Interpretation}: A 1 SD $\uparrow$ in stringency is associated with roughly $10-12$\% lower multifamily density.
