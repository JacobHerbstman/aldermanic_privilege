...
\node[unobserved, text width=3.4cm] (s1) at (-0.2, 0) {
    \textbf{Pre-application}\\[1pt]
    {\scriptsize\textit{Meet with planning}}\\[-2pt]
    {\scriptsize\textit{dept.\ (DPD) +}}\\[-2pt]
    {\scriptsize\textit{community engagement}}
};
...
\visible<5->{
    ...
    \node[font=\tiny, text=pipeobs] at ([yshift=-12pt]mid34 |- bracket)
        {Observable in permit data $\;\rightarrow\;$ measure of regulatory delay};
}
...
\only<1>{
    \node[font=\scriptsize, text=black!70, text width=7cm, align=left, anchor=north] (q1) at (qspot) {
        ...
        Developers meet the Department of Planning and Development, the ward's alderman and neighbors before filing. Projects discouraged here are never proposed.
    };
}
