# setwd("tasks/construction_commercial_measurements/code")

source("../../setup_environment/code/packages.R")
source("../../shared/code/save_data.R")


# Select commercial evidence rules
...
vintages <- readr::read_csv(
  "../input/commercial_family_vintage_summary.csv", show_col_types = FALSE,
...
linked <- read_csv("../input/project_permit_chain_links.csv",
...
address <- read_csv("../input/commercial_address_permit_history.csv",
  col_types = cols(project_id = "c", permit_id = "c", permit_number = "c", .default = col_guess()),
  show_col_types = FALSE) %>%
  transmute(project_id, permit_id, permit_number, permit_type, issue_date = as.Date(issue_date),
    work_description = str_squish(coalesce(work_description, "")), linked = TRUE)
spatial <- read_csv("../input/new_construction_spatial_permit_matches.csv",
  col_types = cols(project_id = "c", permit_id = "c", permit_number = "c", .default = col_guess()),
  show_col_types = FALSE) %>%
...
land <- readr::read_csv(
  "../input/preferred_project_boundary_scope.csv", show_col_types = FALSE,
...
scope <- readr::read_csv(
  "../input/preferred_adjudication_scope.csv", show_col_types = FALSE,
...
# Build preferred commercial ledger
...
entity_versions <- readr::read_csv(
  "../input/commercial_entity_version_candidates.csv",
  show_col_types = FALSE,
  col_types = readr::cols(
...
stopifnot(!anyDuplicated(reported_land$project_id),
          ...
          all(is.finite(reported_land$land_sqft) & reported_land$land_sqft > 1))
preferred_projects$land_source_row_ids <- NA_character_
map_land <- preferred_projects$land_source %in% c(
  "construction_year_parcel_union", "construction_year_parcel_polygon",
  "construction_year_union_of_2021_components"
)
for (i in which(map_land)) {
  project <- preferred_projects[i, ]
  target_pins <- sort(strsplit(project$component_pins, "/", fixed = TRUE)[[1]])
  candidate <- candidates %>% filter(project_id == project$project_id)
  earlier <- entity_versions %>%
    filter(project_family_id == project$project_id, valuation_year == 2021)
  earlier_pins <- str_remove_all(unlist(strsplit(earlier$pins, ",")), "[^0-9]")
  complete_earlier <- nrow(earlier) > 0 && !anyDuplicated(earlier_pins) &&
    setequal(earlier_pins, target_pins) && all(is.finite(earlier$landsf) & earlier$landsf > 1)
  same_candidate <- nrow(candidate) == 1 &&
  ...
  if (nrow(external) == 1) {
    ...
    preferred_projects$land_sqft[i] <- sum(earlier$landsf)
    preferred_projects$land_source[i] <- "commercial_2021_exact_disjoint_reported_lots"
    preferred_projects$land_source_row_ids[i] <- paste(sort(earlier$raw_row), collapse = "/")
    # A later rollup can repeat one component's floor area. Use the complete
    # older floor-area total only when its unit total matches the selected count.
    if (nrow(earlier) > 1) {
      complete_building <- all(is.finite(earlier$bldgsf) & earlier$bldgsf > 1) &&
        all(is.finite(earlier$reported_units)) &&
        sum(earlier$reported_units) == project$dwelling_units
      if (complete_building) {
        preferred_projects$building_sqft[i] <- sum(earlier$bldgsf)
        preferred_projects$building_source[i] <- "commercial_2021_complete_component_sum"
      } else if (all(is.finite(earlier$bldgsf)) &&
                 sum(earlier$bldgsf) == project$building_sqft) {
        # Previously reviewed unit corrections do not invalidate an unchanged
        # floor-area total that already includes every component.
        preferred_projects$building_source[i] <- "commercial_2021_complete_component_sum"
      } else {
  ...
  if (!is.finite(preferred_projects$land_sqft[i])) {
    preferred_projects$allow_far[i] <- FALSE
    preferred_projects$allow_dupac[i] <- FALSE
    preferred_projects$unresolved_caveat[i] <- "No source reports land for the complete selected property; excluded from density."
  }
}
stopifnot(!any(preferred_projects$land_source %in% c(
  "construction_year_parcel_union", "construction_year_parcel_polygon",
  "construction_year_union_of_2021_components"
)))
