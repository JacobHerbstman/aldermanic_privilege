...
active_unresolved_projects <- unresolved_inventory %>%
  anti_join(
    candidates %>% filter(candidate_status == "exclude_outside_period"),
    by = "project_id"
  )
if (!setequal(active_unresolved_projects$project_id, dispositions$source_project_id)) {
  stop("The source disposition ledger must cover unresolved projects still in the candidate period.", call. = FALSE)
}
