...
# Aggregate each PIN-year once before linking it to successor projects. A
# predecessor parcel may legitimately support several later project requests.
connection <- DBI::dbConnect(duckdb::duckdb())
DBI::dbWriteTable(
  connection, "requested_predecessors",
  requests |> dplyr::distinct(predecessor_pin14),
...
history <- DBI::dbGetQuery(
  ...
  paste(
    ...
    "header = true, all_varchar = true, ignore_errors = false) h",
    "SEMI JOIN requested_predecessors r ON h.pin = r.predecessor_pin14"
  )
) |> tibble::as_tibble()
...
history <- history |>
  dplyr::mutate(
    ...
    single_family_record = stringr::str_detect(
      dplyr::coalesce(residence_use, ""), stringr::regex("^single", ignore_case = TRUE)
    ) | residence_type %in% c("1 Story", "1.5 Story", "2 Story", "3 Story +", "Split Level"),
    dwelling_units = dplyr::case_when(
      is.finite(parsed_apartments) & parsed_apartments > 0 ~ parsed_apartments,
      single_family_record ~ 1,
      TRUE ~ NA_real_
    )
  ) |>
  dplyr::filter(is.finite(tax_year), is.finite(building_sqft), building_sqft > 0) |>
  dplyr::select(pin, tax_year, card, reported_year_built, building_sqft, dwelling_units) |>
  dplyr::distinct()
if (anyDuplicated(history[c("pin", "tax_year", "card")])) {
  stop("Conflicting predecessor records share a PIN, assessment year, and card.")
}
snapshots <- history |>
  dplyr::group_by(pin, tax_year) |>
  dplyr::summarise(
    reported_year_values = list(sort(unique(reported_year_built[is.finite(reported_year_built)]))),
    predecessor_building_sqft = sum(building_sqft),
    predecessor_dwelling_units = sum(dplyr::if_else(
      is.finite(dwelling_units) & dwelling_units > 0, dwelling_units, 1
    )),
    .groups = "drop"
  )
selected_snapshots <- requests |>
  dplyr::distinct(predecessor_pin14, construction_year) |>
  dplyr::inner_join(
    snapshots,
    by = dplyr::join_by(predecessor_pin14 == pin, closest(construction_year > tax_year)),
    relationship = "many-to-one"
  )
predecessor_evidence <- requests |>
  dplyr::inner_join(
    selected_snapshots, by = c("predecessor_pin14", "construction_year"),
    relationship = "many-to-one"
  ) |>
  dplyr::group_by(project_id) |>
  dplyr::summarise(
    predecessor_pins_with_structures = dplyr::n_distinct(predecessor_pin14),
    predecessor_latest_tax_year = max(tax_year),
    predecessor_reported_years = paste(sort(unique(unlist(reported_year_values))), collapse = "/"),
    predecessor_building_sqft = sum(predecessor_building_sqft),
    predecessor_dwelling_units = sum(predecessor_dwelling_units),
    .groups = "drop"
  )
...
readr::write_csv(evidence, "../output/predecessor_assessor_evidence.csv", na = "")
