...
history <- readr::read_csv(
  ...
  mutate(
    units_text = str_to_lower(str_squish(apartments_text)),
    parsed_units = case_when(
      is.na(units_text) | units_text == "" ~ NA_real_,
      units_text %in% c("none", "zero") ~ 0,
      units_text == "one" ~ 1,
      units_text == "two" ~ 2,
      units_text == "three" ~ 3,
      units_text == "four" ~ 4,
      units_text == "five" ~ 5,
      units_text == "six" ~ 6,
      TRUE ~ suppressWarnings(as.numeric(str_replace_all(units_text, "[^0-9.-]", "")))
    ),
    dwelling_units = case_when(
      is.finite(parsed_units) & parsed_units > 0 ~ parsed_units,
      str_detect(single_v_multi_family, regex("^single", ignore_case = TRUE)) |
        type_of_residence %in% c("1 Story", "1.5 Story", "2 Story", "3 Story +", "Split Level") ~ 1,
      TRUE ~ NA_real_
    )
  )
...
stopifnot(!anyDuplicated(project_evidence$project_id),
  !anyDuplicated(episodes[c("project_id", "tax_year", "construction_year")]))
readr::write_csv(episodes, "../output/residential_tieback_construction_episode_evidence.csv")
readr::write_csv(project_evidence, "../output/residential_tieback_card_project_evidence.csv")
