...
component_geometry <- sf::st_read(
  ...
  inner_join(single_component_projects, by = c("project_id", "component_pin"), relationship = "one-to-one") %>%
  inner_join(retained_mechanical, by = c("project_id", "target_year" = "construction_year"), relationship = "one-to-one")
...
ward_panel <- sf::st_read("../input/ward_panel.gpkg", quiet = TRUE) %>%
  sf::st_transform(3435)
ward_maps <- load_canonical_ward_maps(ward_panel, eras = sort(unique(accepted_points$era)))
boundary_lines <- load_boundary_layers("../input/ward_pair_boundaries.gpkg", eras = sort(unique(accepted_points$era)))
...
readr::write_csv(review, "../output/residential_shared_site_location_review.csv")
