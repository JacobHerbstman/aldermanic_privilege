...
if (
  !all(manual_review_projects$project_id %in% manual_episode_decisions$project_id) ||
    !all(manual_episode_decisions$project_id %in% review$project_id) ||
    ...
    any(
      manual_episode_decisions |>
        dplyr::inner_join(
          review |> dplyr::select(project_id, component_id),
          by = "project_id",
  ...
  missing_decisions <- setdiff(manual_review_projects$project_id, manual_episode_decisions$project_id)
  stop(paste0(
    "Manual episode decisions do not cover the current grouping. Missing: ",
    paste(missing_decisions, collapse = ", "),
    ". Check component IDs and decision values before continuing."
  ), call. = FALSE)
}
if (
...
adjudication <- review |>
  ...
  dplyr::mutate(
    # An explicit card-inventory decision governs values even when the
    # project also satisfies a broader automatic successor rule.
    final_units = dplyr::if_else(
      episode_disposition %in% "retain_card_inventory",
      summed_card_units, final_units
    ),
    final_building_sqft = dplyr::if_else(
      episode_disposition %in% "retain_card_inventory",
      summed_card_building_sqft, final_building_sqft
    ),
    adjudication_reason = dplyr::if_else(
...
suppression_parents <- adjudication |>
  ...
  dplyr::filter(
    final_disposition == "retain",
    (is.na(successor_rule) & review_class %in% c(
      "successor_episode_exact_reproduction",
      "successor_episode_units_reproduced",
      "complete_card_successor_reproduction"
...
ward_maps <- load_canonical_ward_maps(ward_panel, eras = sort(unique(override_points$era)))
boundary_lines <- load_boundary_layers(
  "../input/ward_pair_boundaries.gpkg", eras = sort(unique(override_points$era))
)
