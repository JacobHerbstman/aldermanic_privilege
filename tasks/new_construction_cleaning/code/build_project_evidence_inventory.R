...
permit_units <- readr::read_csv(
  "../output/new_construction_permit_unit_mentions.csv",
  show_col_types = FALSE
) |>
  dplyr::rename(source_project_id = project_id) |>
  dplyr::inner_join(
...
evidence <- ledger |>
  ...
  dplyr::mutate(
    ...
    externally_reviewed = review_status %in% "complete",
    ...
    classification_signal = dplyr::case_when(
      classification_source == "assessor_card_rule" & multifamily_disposition == "include" ~
        "assessor_cards_multifamily",
      classification_source == "assessor_card_rule" & multifamily_disposition == "exclude" ~
        "assessor_cards_single_family",
