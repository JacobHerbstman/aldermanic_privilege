...
queue <- adjudication |>
  ...
  dplyr::mutate(
    review_status = dplyr::coalesce(review_status, "not_reviewed"),
    classification_source = dplyr::if_else(
      !is.na(classification_override), "recorded_exception", "assessor_card_rule"
    ),
    ...
    reviewed_disposition = dplyr::case_when(
      review_status != "complete" ~ NA_character_,
    ...
    multifamily_disposition = dplyr::case_when(
      review_status == "lineage_pending" ~ "pending",
      !is.na(classification_override) ~ classification_override,
      building_type_rule == "suppressed" ~ "suppress",
      building_type_rule == "multifamily_building" ~ "include",
      building_type_rule == "single_family_buildings" ~ "exclude",
      TRUE ~ "unresolved"
    )
  ) |>
  dplyr::select(
...
if (
  nrow(queue) != nrow(adjudication) ||
    ...
    any(!is.na(queue$classification_override) &
      (!queue$classification_override %in% c("include", "exclude", "suppress") |
        is.na(queue$reviewed_disposition) |
        queue$classification_override != queue$reviewed_disposition)) ||
