...
historical_coordinates <- read_csv(
  "../output/density_historical_coordinates.csv",
  show_col_types = FALSE,
...
if (anyNA(residential$pin) || anyDuplicated(residential$pin) ||
    anyNA(multifamily$pin) || anyDuplicated(multifamily$pin)) {
  stop("Residential and commercial inputs must each have unique PINs.")
}
...
if (any(historical_coordinates$pin %in% parcels$pin)) {
  stop("A accepted historical coordinate conflicts with a complete 2025 parcel coordinate.", call. = FALSE)
}
...
if (nrow(historical_output) != nrow(historical_coordinates) ||
  ...
  stop("Not every accepted historical coordinate appears exactly once in the geocoded output.", call. = FALSE)
}
