...
ward_panel <- sf::st_read("../input/ward_panel.gpkg", quiet = TRUE) %>%
  sf::st_transform(3435)
ward_maps <- load_canonical_ward_maps(ward_panel, eras = sort(unique(centroids$era)))
boundary_lines <- load_boundary_layers("../input/ward_pair_boundaries.gpkg", eras = sort(unique(centroids$era)))
