...
match_summary <- readr::read_csv(
  "../output/multicard_successor_match_summary.csv", show_col_types = FALSE
) |>
  dplyr::select(project_id, complete_unambiguous_successor_inventory)
...
review <- parents |>
  dplyr::left_join(match_summary, by = "project_id", relationship = "one-to-one") |>
  dplyr::left_join(
    component_nodes,
    by = "project_id",
  ...
  dplyr::mutate(
    review_class = dplyr::case_when(
      ...
      inventory_same_year_observed & all_target_card_units_observed &
        all_target_card_building_sqft_observed &
        complete_unambiguous_successor_inventory ~
    ...
    preliminary_project_units = dplyr::case_when(
      ...
      review_class ==
        "aggregate_cards_supported_no_successor_overlap" ~
        summed_card_units,
      review_class == "complete_card_successor_reproduction" ~ summed_card_units,
      review_class %in% c(
        "successor_episode_exact_reproduction",
        "successor_episode_units_reproduced"
      ) ~ terminal_child_units,
    ...
    preliminary_project_building_sqft = dplyr::case_when(
      ...
      review_class == "complete_card_successor_reproduction" ~ summed_card_building_sqft,
      review_class %in% c(
        "successor_episode_exact_reproduction",
        "successor_episode_units_reproduced"
      ) ~ terminal_child_building_sqft,
