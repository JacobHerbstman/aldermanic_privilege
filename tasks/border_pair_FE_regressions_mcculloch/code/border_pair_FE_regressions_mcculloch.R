...
message(sprintf("\n=== Border-Pair FE McCulloch Configuration ==="))
...
if (!fe_spec %in% names(fe_formulas)) {
  stop(sprintf("Invalid fe_spec '%s'. Must be one of: %s", fe_spec, paste(names(fe_formulas), collapse = ", ")), call. = FALSE)
}
...
for (yv in yvars) {
  ...
  if (!base_var %in% names(parcels_fe)) {
    warning(sprintf("Skipping '%s' (base var '%s' not found).", yv))
    next
  }
...
fe_rows[["_Dep. Var. Mean"]] <- vapply(models, function(x) sprintf("%.2f", mean(x$custom_data[[gsub("^log\\(|\\)$", "", all.vars(formula(x))[1])]], na.rm = TRUE)), character(1))
