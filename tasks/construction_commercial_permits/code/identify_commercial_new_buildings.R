# setwd("tasks/construction_commercial_permits/code")

source("../../shared/code/save_data.R")
source("../../setup_environment/code/packages.R")


# Build commercial city building footprints

library(sf)
library(dplyr)
library(stringr)

projects <- st_read("../input/preferred_project_year_geometry.gpkg", quiet = TRUE) |>
  st_transform(3435) |>
...
# Build commercial ground up evidence
...
address_permits <- sf::st_read("../input/building_permits_for_verification.gpkg", quiet = TRUE) %>%
  sf::st_drop_geometry() %>%
  filter(
