...
census_block_groups <- census_blocks %>%
  ...
  mutate(year = list(seq(start_year, end_year))) %>%
  unnest(year) %>%
  mutate(block_id = as.character(block_group_id)) %>% 
  dplyr::select(year, block_group_id, geometry) %>% 
...
block_groups_by_year <- group_split(census_block_groups, year)
wards_by_year  <- group_split(ward_panel, year)


joined_panel <- map2_dfr(
  block_groups_by_year,
  wards_by_year,
  ~ {
    # Add a column with the total area of each original block group
    block_groups_with_area <- .x %>%
      mutate(total_bg_area = st_area(geometry))
    
    # Find the intersection between block groups and all overlapping wards
    intersection <- st_intersection(block_groups_with_area, .y)
    
    intersection %>%
      mutate(
        intersection_area = st_area(geometry),
        # NEW: Calculate the proportion of the block group in this ward
        area_proportion = as.numeric(intersection_area / total_bg_area)
      ) %>%
      group_by(block_group_id) %>%
      ...
      select(
        ...
        block_group_id,
        ward,
        area_proportion # NEW: Keep the proportion column
      )
  }
) %>%
  arrange(block_group_id,ward, year) 

## keep only "clean" ward switches
clean_panel <- joined_panel %>%
  group_by(block_group_id) %>%
  filter(
    all(area_proportion[year < 2015] > 0.99) &
      all(area_proportion[year >= 2015] > 0.99)
  ) %>%
  ungroup()


joined_panel_switch <- clean_panel %>%
...
##write to shapefile
st_write(joined_panel_switch,"../output/census_block_groups_ward_switchers.shp", append = F, delete_dsn = TRUE)
...
## map 
  ...
  # labs(
  #   title = "Chicago Wards and Census Block Groups",
  #   subtitle = "Wards outlined in black, Census Block Groups in blue",
  #   x = NULL, y = NULL
  # )
#
