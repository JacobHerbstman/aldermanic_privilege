...
# private_yearly_input <- "../input/home_sales_geography_yearly_summary.csv"
...
# yearly_panel_output <- "../output/home_sales_acs_yearly_panel.csv"
# growth_output <- "../output/home_sales_acs_growth_comparison.csv"
# diagnostics_output <- "../output/home_sales_acs_diagnostics.csv"
# coverage_output <- "../output/home_sales_acs_coverage_summary.csv"
...
acs_panel <- acs_bundle$panel[
  ...
  .(
    ...
    geography_name,
    year,
    value_real = acs_home_value_real_2022
  )
]
acs_panel[, `:=`(
  series_id = "acs_home_value",
  series_label = "ACS owner-occupied home value",
...
private_yearly <- fread(private_yearly_input)[
  ...
  .(
    ...
    value_real = as.numeric(median_sale_price_real_2022),
    n_obs = as.integer(n_transactions)
  )
]
private_yearly[, `:=`(
  series_id = "private_home_sales",
  series_label = "Private home sales",
  base_year = fifelse(geography_level == "ward", 2015L, 2014L)
)]
...
setorder(growth_comparison, geography_level, geography_id)
