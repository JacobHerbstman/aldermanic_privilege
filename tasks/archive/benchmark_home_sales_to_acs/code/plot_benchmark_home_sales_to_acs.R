...
# setwd("/Users/jacobherbstman/Desktop/aldermanic_privilege/tasks/benchmark_home_sales_to_acs/code")
# yearly_panel_input <- "../output/home_sales_acs_yearly_panel.csv"
# growth_input <- "../output/home_sales_acs_growth_comparison.csv"
# ward_panel_input <- "../input/ward_panel.gpkg"
# community_area_input <- "../input/community_areas.geojson"
# citywide_plot_output <- "../output/home_sales_acs_citywide_index_2014_2019.pdf"
# ward_scatter_output <- "../output/home_sales_acs_scatter_ward_2015_2019.pdf"
# community_scatter_output <- "../output/home_sales_acs_scatter_community_area_2014_2019.pdf"
# ward_map_output <- "../output/home_sales_acs_map_ward_2015_2019.pdf"
# community_map_output <- "../output/home_sales_acs_map_community_area_2014_2019.pdf"
...
series_colors <- c(
  "ACS owner-occupied home value" = "#1b1b1b",
  "Private home sales" = "#b15928"
)
...
citywide_plot <- ggplot(
  ...
  labs(
    ...
    y = "Index (2014 = 100)",
    color = NULL,
    title = "Chicago Real Home-Value Growth, ACS vs Private Home Sales",
...
save_scatter <- function(target_geography_level, output_path, title_text) {
  scatter_dt <- growth_comparison[geography_level == target_geography_level]
  ...
  scatter_plot <- ggplot(
    ...
    geom_point(color = "#b15928", alpha = 0.8, size = 2.2) +
    labs(
      x = "ACS growth (%)",
      y = "Private home-sales growth (%)",
      title = title_text
    ) +
    theme_minimal(base_size = 12)
...
save_scatter("ward", ward_scatter_output, "Ward Home-Value Growth: ACS vs Private Home Sales (2015-2019)")
save_scatter("community_area", community_scatter_output, "Community-Area Home-Value Growth: ACS vs Private Home Sales (2014-2019)")
...
save_map <- function(target_geography_level, map_year, output_path, title_text) {
  ...
  map_long <- rbindlist(
    list(
      map_dt[, .(geography_id, geography_name, series_label = "ACS owner-occupied home value", growth_pct = acs_growth_pct)],
      map_dt[, .(geography_id, geography_name, series_label = "Private home sales", growth_pct = private_growth_pct)]
    )
  )
...
save_map("ward", 2015, ward_map_output, "Ward Home-Value Growth, ACS vs Private Home Sales (2015-2019)")
save_map("community_area", 2019, community_map_output, "Community-Area Home-Value Growth, ACS vs Private Home Sales (2014-2019)")
