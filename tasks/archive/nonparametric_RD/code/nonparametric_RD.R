...
bins_mi <- bins / 5280
cat(sprintf(
  "→ Nonparametric stacked RD | y=%s | log=%s | bw=%.0fft (%.2f mi) | bin=%.1fft (%.2f mi)\n",
...
tidy_extract <- function(est, se_label) {
  broom::tidy(est) %>%
    ...
    dplyr::mutate(
      bin = suppressWarnings(as.integer(str_remove(term, "bindex::"))),
      {{ se_label }} := std.error
    ) %>%
...
stars <- if (is.na(rd_p_exact)) "" else if (rd_p_exact <= 0.01) "***" else if (rd_p_exact <= 0.05) "**" else if (rd_p_exact <= 0.10) "*" else ""
...
p <- ggplot() +
  ...
  geom_ribbon(
    data = bins_L, aes(x = bin_center_mi, ymin = lo95, ymax = hi95),
    fill = col_left, alpha = 0.25, color = NA
  ) +
  geom_ribbon(
    data = bins_R, aes(x = bin_center_mi, ymin = lo95, ymax = hi95),
    fill = col_right, alpha = 0.25, color = NA
  ) +
  # lines (clustered coefficients)
  geom_line(
    data = bins_L, aes(x = bin_center_mi, y = coef_clu),
    linewidth = 1, color = col_left
  ) +
  geom_line(
    data = bins_R, aes(x = bin_center_mi, y = coef_clu),
