...
# Event study with different fixed effects specifications (RENTAL VERSION)
...
parser <- add_option(parser, c("-w", "--weighted"),
    ...
    help = "Weight by number of listings [default: FALSE]"
)
...
data <- read_csv("../input/rental_stacked_panel.csv", show_col_types = FALSE) %>%
    ...
    mutate(
        ...
        # Extract block_group_id from block_id (first 12 digits)
