...
# setwd("/Users/jacobherbstman/Desktop/aldermanic_privilege/tasks/audits/slide_deck_audit/code")
...
run_geometry_checks <- function() {
  ...
  sales_panel <- read_csv(file.path(root_dir, "tasks/merge_sales_scores/output/sales_with_ward_distances_through2022.csv"), show_col_types = FALSE)
  rent_panel <- arrow::read_parquet(file.path(root_dir, "tasks/merge_rent_scores/output/rent_with_ward_distances_full_through2022.parquet"))
  ...
  checks <- tribble(
    ...
    "sales_signed_distance_consistency", mean(sign(sales_panel$signed_dist) == sales_panel$sign, na.rm = TRUE), 0.99, ">= 0.99", ifelse(mean(sign(sales_panel$signed_dist) == sales_panel$sign, na.rm = TRUE) >= 0.99, "verified", "mismatch"), "tasks/merge_sales_scores/output/sales_with_ward_distances_through2022.csv", "Sales signed distance should agree with sign variable",
    "rent_signed_distance_consistency", mean(sign(rent_panel$signed_dist) == rent_panel$sign, na.rm = TRUE), 0.99, ">= 0.99", ifelse(mean(sign(rent_panel$signed_dist) == rent_panel$sign, na.rm = TRUE) >= 0.99, "verified", "mismatch"), "tasks/merge_rent_scores/output/rent_with_ward_distances_full_through2022.parquet", "Rental signed distance should agree with sign variable",
