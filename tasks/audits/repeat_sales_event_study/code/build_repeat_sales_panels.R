...
if (length(args) == 0) {
  args <- c(panel_mode, approach)
}

if (length(args) != 2) {
  stop(
    "Usage: Rscript build_repeat_sales_panels.R <panel_mode> <approach>",
    call. = FALSE
  )
}
...
output_panel <- sprintf("../output/repeat_sales_panel_%s_%s.parquet", panel_mode, approach)
