...
# setwd("/Users/jacobherbstman/Desktop/aldermanic_privilege/tasks/audits/repeat_sales_event_study/code")
...
if (length(args) == 0) {
  args <- c(panel_mode, approach, treatment_mode)
}

if (length(args) != 3) {
  stop(
    "Usage: Rscript run_repeat_sales_event_study.R <panel_mode> <approach> <treatment_mode>",
    call. = FALSE
  )
}
...
input_panel <- sprintf("../output/repeat_sales_panel_%s_%s.parquet", panel_mode, approach)
plot_pdf <- sprintf("../output/repeat_sales_event_study_%s_%s_%s.pdf", panel_mode, approach, treatment_mode)
combined_pdf <- sprintf("../output/repeat_sales_event_study_combined_%s_%s_%s.pdf", panel_mode, approach, treatment_mode)
coefficients_csv <- sprintf("../output/repeat_sales_coefficients_%s_%s_%s.csv", panel_mode, approach, treatment_mode)
support_csv <- sprintf("../output/repeat_sales_support_%s_%s_%s.csv", panel_mode, approach, treatment_mode)
pretrend_csv <- sprintf("../output/repeat_sales_pretrend_%s_%s_%s.csv", panel_mode, approach, treatment_mode)
metadata_csv <- sprintf("../output/repeat_sales_metadata_%s_%s_%s.csv", panel_mode, approach, treatment_mode)
