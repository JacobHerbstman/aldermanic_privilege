# setwd("tasks/audits/commercial_construction_review/code")
...
components <- readr::read_csv(
  "../input/preferred_commercial_project_components.csv",
  show_col_types = FALSE,
...
if (anyNA(components$component_pin) || anyDuplicated(components$component_pin)) {
  stop("Commercial candidate components must be unique by PIN before joining assessment history.")
}
...
SaveData(project_evidence, character(), "../output/commercial_residential_history_evidence.csv")
