...
pd_numbers = sorted(
    queue.loc[queue["pd_number"].notna() & queue["pd_number"].ne(""), "pd_number"].unique(),
    key=int,
)
...
pd.DataFrame(page_rows).to_csv("../output/pd_archive_text.csv", index=False)
pd.DataFrame(field_rows).to_csv("../output/pd_archive_fields.csv", index=False)
