...
pages = pd.read_csv("../output/pd_archive_text.csv", dtype=str, low_memory=False)
fields = pd.read_csv("../output/pd_archive_fields.csv", dtype=str)

if fields["pd_number"].duplicated().any():
    raise ValueError("DPD archive fields contain duplicate PD numbers")
if not fields["download_status"].eq("downloaded").all():
    raise ValueError("At least one requested DPD archive failed to download")
if pages.duplicated(["pd_number", "page_number"]).any():
    raise ValueError("DPD archive text contains duplicate PD/page rows")
...
if not parsed_page_counts.equals(expected_page_counts.sort_index()):
    raise ValueError("DPD archive text does not match the reported page counts")
...
evidence.to_csv("../output/pd_archive_far_evidence.csv", index=False)
...
review.to_csv("../output/pd_archive_review_queue.csv", index=False)
