...
if (anyNA(buildings$pin) || anyDuplicated(buildings$pin)) {
  stop("Historical address requests must have unique PINs.")
}
...
for (pin_chunk in pin_chunks) {
    ...
    parameters <- c(
      "$select" = "pin,pin10,year,prop_address_full,prop_address_city_name,prop_address_state,prop_address_zipcode_1,row_id",
      "$where" = sprintf(
    ...
    if (is.null(response) || response$status_code != 200L) {
      stop(
        sprintf(
          "Historical address query %d of %d failed: %s",
    ...
    if (request_number %% 10L == 0L || request_number == total_requests) {
      message(sprintf("Historical address requests: %d of %d", request_number, total_requests))
    }
}
...
if (nrow(duplicate_records) > 0) {
  stop("Historical Parcel Addresses returned duplicate PIN-year records.", call. = FALSE)
}
...
if (anyNA(historical$pin) || anyNA(historical$year) ||
  ...
  stop("Historical Parcel Addresses returned unrequested PIN-year records.", call. = FALSE)
}
