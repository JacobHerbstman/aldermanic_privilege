...
query_object_ids <- function(layer_url, points) {
  ...
  if (inherits(response, "error")) {
    if (nrow(points) == 1L) {
      stop(
        "Nearest-parcel object-ID query failed for request ",
        points$request_id,
        ": ",
  ...
  if (!is.null(result$error)) {
    stop(
      "Nearest-parcel object-ID query failed: ",
...
chains_without_exact_pin <- exact_matches |>
  ...
  dplyr::filter(!has_exact_pin) |>
  dplyr::select(permit_chain_id)

requests <- do.call(rbind, lapply(0:4, function(year_offset) {
  chain_points |>
    dplyr::semi_join(
      chains_without_exact_pin,
      by = "permit_chain_id"
    ) |>
    dplyr::inner_join(
      chains |>
    ...
    dplyr::mutate(
      ...
      request_id = paste(permit_chain_id, target_year, sep = ":")
    ) |>
    dplyr::select(
      request_id,
      permit_chain_id,
...
if (anyDuplicated(requests$request_id) ||
  ...
  stop("Nearest historical-parcel requests are invalid.", call. = FALSE)
}
...
for (year_value in sort(unique(requests$target_year))) {
  ...
  if (length(year_parcels) == 0) {
    nearest_matches[[as.character(year_value)]] <- year_requests |>
      sf::st_drop_geometry() |>
      dplyr::mutate(
        candidate_polygon_count = 0L,
        nearest_tie_count = 0L,
        nearest_distance_ft = NA_real_,
        nearest_match_status = "no_parcel_within_100ft"
      )
    next
  }
  ...
  if (anyDuplicated(year_parcels$object_id) ||
    ...
    stop(
      "Nearest historical parcel geometries are invalid for ",
      year_value,
      call. = FALSE
    )
  }
  ...
  year_matches <- purrr::map_dfr(
    ...
    function(request_row) {
      ...
      candidate_distances <- distance_matrix[
        request_row,
        candidate_rows
      ]
      minimum_distance <- min(candidate_distances)
      nearest_rows <- candidate_rows[
        candidate_distances <= minimum_distance + tie_tolerance_ft
      ]
      dplyr::bind_cols(
        request[rep(1, length(nearest_rows)), ],
        sf::st_drop_geometry(year_parcels[nearest_rows, ]) |>
          dplyr::select(
            object_id,
            historical_pin14,
            historical_pin10
        ...
        tibble::tibble(
          candidate_polygon_count = length(candidate_rows),
          nearest_tie_count = length(nearest_rows),
          nearest_distance_ft = distance_matrix[
            request_row,
            nearest_rows
          ],
          nearest_match_status = dplyr::if_else(
            length(nearest_rows) == 1L,
            "unique_nearest_parcel",
            "ambiguous_nearest_parcels"
          )
        )
      )
    }
  ...
  nearest_matches[[as.character(year_value)]] <- year_matches
...
nearest_matches <- dplyr::bind_rows(nearest_matches) |>
  dplyr::arrange(
    target_year,
    permit_chain_id,
    request_id,
...
) || anyDuplicated(nearest_matches[c("request_id", "object_id")])) {
  stop("Nearest historical parcel outputs contain duplicate keys.", call. = FALSE)
}

chain_coverage <- chains_without_exact_pin |>
  dplyr::left_join(
    nearest_matches |>
      dplyr::filter(
      ...
      dplyr::group_by(permit_chain_id) |>
      dplyr::summarise(
        recovered_year_requests = dplyr::n_distinct(target_year),
        recovered_historical_pins = dplyr::n_distinct(
          historical_pin14,
          na.rm = TRUE
        ),
        minimum_distance_ft = min(nearest_distance_ft),
        maximum_distance_ft = max(nearest_distance_ft),
  ...
  dplyr::mutate(
    recovered_year_requests = dplyr::coalesce(
      recovered_year_requests,
      0L
    ),
...
summary <- dplyr::bind_rows(
  nearest_matches |>
    dplyr::distinct(request_id, nearest_match_status) |>
    dplyr::count(nearest_match_status, name = "value") |>
    dplyr::transmute(
      section = "request_status",
      metric = nearest_match_status,
      value
    ),
  tibble::tibble(
    section = "coverage",
    metric = c(
      "chains_without_exact_pin",
      "fallback_reference_point_requests",
      "chains_with_unique_nearest_pin",
      "chains_without_unique_nearest_pin",
      "unique_nearest_historical_pins",
      "downloaded_candidate_parcels"
    ),
    value = c(
      nrow(chains_without_exact_pin),
      dplyr::n_distinct(nearest_matches$request_id),
      sum(chain_coverage$recovered_historical_pins > 0),
      sum(chain_coverage$recovered_historical_pins == 0),
      dplyr::n_distinct(nearest_matches$historical_pin14[
        nearest_matches$nearest_match_status ==
          "unique_nearest_parcel" &
          stringr::str_length(nearest_matches$historical_pin14) == 14L
      ]),
...
readr::write_csv(
  chain_coverage,
  "../output/residual_permit_nearest_historical_parcel_chain_coverage.csv"
)
readr::write_csv(
  dplyr::bind_rows(layer_manifest),
  "../output/residual_permit_nearest_historical_parcel_manifest.csv"
)
readr::write_csv(
  summary,
  "../output/residual_permit_nearest_historical_parcel_summary.csv"
)
