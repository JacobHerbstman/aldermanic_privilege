...
scope <- readr::read_csv(
  ...
  mutate(study_period_year_values = coalesce(study_period_year_values, "")) %>%
  filter(study_period_year_values != "") %>%
  select(
    project_id,
    project_kind,
    component_pins,
...
if (anyDuplicated(scope$project_id) > 0 ||
    anyDuplicated(history[c("project_id", "pin", "tax_year", "card_num")]) > 0 ||
    !setequal(scope$project_id, history$project_id)) {
  stop("Unresolved card-evidence inputs violate their declared keys or coverage.", call. = FALSE)
}

project_year <- history %>%
  group_by(project_id, tax_year) %>%
  mutate(any_positive_card_proration = any(card_proration_rate > 0, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(
    building_key = paste0(
      "card", card_num,
      ...
      "|type", coalesce(residence_type, "missing"),
      "|use", coalesce(residence_use, "missing")
    )
  )

site_land <- project_year %>%
...
active_cards <- project_year %>%
  ...
  summarise(
    ...
    property_classes = paste(sort(unique(property_class)), collapse = "/"),
    ...
    building_sqft = one_finite_value(building_sqft),
    dwelling_units = one_finite_value(dwelling_units),
...
episodes <- active_cards %>%
  ...
  mutate(
    ...
    far_fields_complete = episode_fields_complete & site_land_complete & site_land_sqft > 0,
    dupac_fields_complete = cards_with_complete_fields == physical_cards &
      site_land_complete & site_land_sqft > 0
  )

snapshots <- active_cards %>%
  group_by(project_id, tax_year) %>%
  summarise(
    ...
    study_period_years = paste(sort(unique(construction_year[study_period_card])), collapse = "/"),
  ...
  left_join(
    episodes %>%
      ...
      summarise(
        ...
        selected_all_far_fields_complete = all(far_fields_complete),
        selected_all_dupac_fields_complete = all(dupac_fields_complete),
        .groups = "drop"
      ),
    by = "project_id",
...
if (nrow(project_evidence) != nrow(scope) ||
    anyDuplicated(project_evidence$project_id) > 0 ||
    anyDuplicated(active_cards[c("project_id", "tax_year", "physical_card_id")]) > 0) {
  stop("Unresolved card evidence is incomplete or nonunique.", call. = FALSE)
}

summary <- bind_rows(
  project_evidence %>%
    count(project_kind, evidence_status, name = "value") %>%
    transmute(
      section = "project_status",
      metric = paste(project_kind, evidence_status, sep = ":"),
      value
    ),
  tibble::tibble(
    section = "validation",
    metric = c(
      "projects",
      "projects_with_usable_snapshot",
      "projects_with_one_episode",
      "projects_with_multiple_episodes",
      "projects_without_usable_snapshot",
      "duplicate_project_ids",
      "duplicate_project_year_card_rows"
    ),
    value = c(
      nrow(project_evidence),
      sum(!is.na(project_evidence$selected_tax_year)),
      sum(project_evidence$evidence_status == "one_episode_card_evidence"),
      sum(project_evidence$evidence_status == "multiple_construction_episodes_review"),
      sum(project_evidence$evidence_status == "no_usable_card_snapshot"),
...
readr::write_csv(active_cards, "../output/residential_unresolved_active_cards.csv")
readr::write_csv(episodes, "../output/residential_unresolved_construction_episodes.csv")
readr::write_csv(snapshots, "../output/residential_unresolved_card_snapshots.csv")
readr::write_csv(project_evidence, "../output/residential_unresolved_card_project_evidence.csv")
readr::write_csv(summary, "../output/residential_unresolved_card_evidence_summary.csv")
