...
if (anyDuplicated(projects[c("project_id", "target_year")]) > 0 ||
    nrow(projects) != nrow(review_scope)) {
  stop("Residential review geometries are not one row per scoped project.", call. = FALSE)
}
if (any(!sf::st_is_valid(projects))) {
  stop("Residential review geometries contain invalid polygons.", call. = FALSE)
}
...
for (batch_number in seq_along(query_batches)) {
  ...
  if (feature_count >= 50000) {
    stop("A residential footprint request reached the Socrata row limit.", call. = FALSE)
  }
  if (feature_count > 0) {
...
if (length(downloaded) == 0) {
  stop("The residential City footprint request returned no features.", call. = FALSE)
}
...
if (anyDuplicated(footprints$footprint_id) > 0 || any(!sf::st_is_valid(footprints))) {
  stop("Downloaded residential City footprints fail uniqueness or geometry checks.", call. = FALSE)
}

summary <- tibble::tibble(
  metric = c(
    "residential_review_projects_requested",
    "request_batches",
...
readr::write_csv(query_index, "../output/residential_review_city_footprint_requests.csv")
readr::write_csv(
  dplyr::bind_rows(manifest),
  "../output/residential_review_city_footprint_manifest.csv"
)
readr::write_csv(summary, "../output/residential_review_city_footprint_summary.csv")
