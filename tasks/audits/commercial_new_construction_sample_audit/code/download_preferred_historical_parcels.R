...
if (any(!historical_parcels$geometry_valid)) {
  stop("Preferred historical parcel download contains invalid geometry.", call. = FALSE)
}
...
summary <- bind_rows(
  ...
  tibble::tibble(
    metric = c(
      ...
      "invalid_historical_parcel_polygons"
    ),
    value = c(
      nrow(coverage),
      ...
      sum(!historical_parcels$geometry_valid)
    )
  )
)
...
readr::write_csv(
  ...
  "../output/preferred_historical_parcel_download_manifest.csv"
)
readr::write_csv(
  summary,
  "../output/preferred_historical_parcel_download_summary.csv"
)
