...
events <- read_csv(
  ...
  filter(
    event_date > as.Date("2014-07-30"),
    event_date <= as.Date("2015-11-18"),
    !is.na(application_key)
  )

zoning_2016$candidate_zone_group_2014 <- zoning_2016$zone_group_2016
zoning_2016$backcast_status <- "anchor_predates_2014_cutoff_or_undated"
zoning_2016$matched_matter_id <- NA_character_

post_cutoff_rows <- which(
  !is.na(zoning_2016$ordinance_date_2016) &
    zoning_2016$ordinance_date_2016 > as.Date("2014-07-30")
)
for (row in post_cutoff_rows) {
  ...
  if (length(candidates) == 0) {
    zoning_2016$candidate_zone_group_2014[row] <- NA_character_
  ...
  if (length(origins) == 1) {
    zoning_2016$candidate_zone_group_2014[row] <- origins[[1]]
...
zoning_2014 <- st_read(
  ...
  st_transform(3435) %>%
  transmute(zone_group_2014 = snapshot_zone_group(ZONE_CLASS), geometry) %>%
  st_make_valid()

polygon_points <- zoning_2016 %>%
  mutate(anchor_polygon_id = row_number()) %>%
  st_point_on_surface()
polygon_matches <- st_intersects(polygon_points, zoning_2014)
...
polygon_check <- polygon_points %>%
  ...
  mutate(
    official_group_count = vapply(
      polygon_matches,
      function(rows) length(unique(zoning_2014$zone_group_2014[rows])),
      integer(1)
    ),
    zone_group_2014 = vapply(
      polygon_matches,
      function(rows) paste(sort(unique(zoning_2014$zone_group_2014[rows])), collapse = ";"),
      character(1)
    ),
    backcast_resolved = !is.na(candidate_zone_group_2014),
    backcast_agrees = backcast_resolved & vapply(
      seq_along(polygon_matches),
      function(row) candidate_zone_group_2014[row] %in%
        zoning_2014$zone_group_2014[polygon_matches[[row]]],
      logical(1)
    )
  )

polygon_check %>%
  write_csv("../output/historical_zoning_2016_to_2014_backcast_polygon_check.csv")
...
polygon_check %>%
  ...
  write_csv("../output/historical_zoning_2016_to_2014_backcast_polygon_summary.csv")
...
projects <- suppressWarnings(
  st_join(projects, zoning_2014, left = TRUE, largest = TRUE)
)
...
projects <- projects %>%
  ...
  mutate(
    ...
    backcast_resolved = !is.na(candidate_zone_group_2014),
    true_group_change = zone_group_2014 != zone_group_2016,
    backcast_agrees = backcast_resolved &
      candidate_zone_group_2014 == zone_group_2014
  )

projects %>%
  write_csv("../output/historical_zoning_2016_to_2014_backcast_project_check.csv")

projects %>%
  filter(!backcast_agrees) %>%
  write_csv("../output/historical_zoning_2016_to_2014_backcast_mismatches.csv")
...
bind_rows(summary_rows) %>%
  write_csv("../output/historical_zoning_2016_to_2014_backcast_summary.csv")
