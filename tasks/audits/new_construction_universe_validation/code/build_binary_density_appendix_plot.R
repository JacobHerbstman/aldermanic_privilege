...
if (length(cli_args) == 0L) {
  cli_args <- c(check_type, score_variant, bins_per_side)
}
if (length(cli_args) != 3L) {
  stop("Expected a check type, score variant, and bins per side.")
}

check_type <- cli_args[1]
score_variant <- cli_args[2]
...
projects <- readr::read_csv(
  ...
  dplyr::mutate(
    ...
    true_side = as.integer(score_own > score_neighbor),
    true_distance_ft = abs(distance_to_boundary_ft) *
      dplyr::if_else(true_side == 1L, 1, -1),
...
ggplot2::ggsave(
  sprintf(
    "../output/binary_density_%s_%s.pdf",
    check_type,
    score_variant
  ),
  combined,
