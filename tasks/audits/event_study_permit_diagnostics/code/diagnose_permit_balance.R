...
# setwd("/Users/jacobherbstman/Desktop/aldermanic_privilege/tasks/audits/event_study_permit_diagnostics/code")
# bandwidth_m <- 250
...
if (length(cli_args) == 0) {
  cli_args <- c(bandwidth_m)
}

if (length(cli_args) != 1) {
  stop("FATAL: Script requires 1 arg: <bandwidth_m>", call. = FALSE)
}
...
balance_output <- sprintf("../output/permit_balance_summary_%s.csv", bandwidth_label)
balance_tex_output <- sprintf("../output/permit_balance_summary_%s.tex", bandwidth_label)
joint_output <- sprintf("../output/permit_balance_joint_summary_%s.csv", bandwidth_label)
joint_tex_output <- sprintf("../output/permit_balance_joint_summary_%s.tex", bandwidth_label)
did_output <- sprintf("../output/permit_balance_did_shift_%s.csv", bandwidth_label)
did_tex_output <- sprintf("../output/permit_balance_did_shift_%s.tex", bandwidth_label)
plot_output <- sprintf("../output/permit_balance_love_plot_%s.pdf", bandwidth_label)
