...
# setwd("/Users/jacobherbstman/Desktop/aldermanic_privilege/tasks/audits/renthub_data_audit/code")
...
if (length(args) == 0) {
  args <- c(raw_dir, out_dir, temp_dir)
}
...
classify_support_status <- function(n_obs) {
  if (!is.finite(n_obs)) {
    return("RED")
  }
  if (n_obs >= 5000) {
...
recommended_status_window <- function(month_start, status) {
  usable <- !is.na(status) & status != "RED"
  if (!any(usable)) {
    return(data.table(window_start = as.Date(NA), window_end = as.Date(NA), n_months = 0L, share_green = NA_real_))
  }
  ...
  candidate[, n_months := end_idx - start_idx + 1L]
  candidate[, share_green := vapply(seq_len(.N), function(i) {
    mean(status[start_idx[i]:end_idx[i]] == "GREEN", na.rm = TRUE)
  }, numeric(1))]
  setorder(candidate, -n_months, -share_green, start_idx)
  top <- candidate[1]
  data.table(
    window_start = month_start[top$start_idx],
    window_end = month_start[top$end_idx],
    n_months = top$n_months,
    share_green = top$share_green
  )
}
...
run_internal_tests <- function() {
  ...
  synthetic_cycle <- data.table(
    analysis_key = c("A", "A", "A", "A"),
    file_date = as.Date(c("2020-01-01", "2020-01-15", "2020-05-01", "2020-05-10"))
  )
  synthetic_cycle[, prev_file_date := shift(file_date), by = analysis_key]
  synthetic_cycle[, gap_days := as.integer(file_date - prev_file_date)]
  synthetic_cycle[, cycle_start_flag := fifelse(is.na(prev_file_date) | gap_days >= 90, 1L, 0L)]
  synthetic_cycle[, cycle_id := cumsum(cycle_start_flag), by = analysis_key]

  test_results <- rbind(
    test_results,
    data.table(
      test_name = "cycle_gap_90_days",
      passed = identical(synthetic_cycle$cycle_id, c(1L, 1L, 2L, 2L)),
      detail = "90-day gaps start a new rental cycle"
    )
  )
...
message("Preparing cleaned day-level benchmark variants...")
dbExecute(
  con,
  "
  ...
  cell_thresholds AS (
    SELECT
      ...
      QUANTILE_CONT(rent_price, 0.02) AS rent_p02,
      QUANTILE_CONT(rent_price, 0.98) AS rent_p98,
      ...
      QUANTILE_CONT(sqft, 0.02) FILTER (WHERE sqft > 0) AS sqft_p02,
      QUANTILE_CONT(sqft, 0.98) FILTER (WHERE sqft > 0) AS sqft_p98,
  ...
  SELECT
    ...
    CASE
      WHEN d.rent_price >= COALESCE(CASE WHEN c.n_obs >= 200 THEN c.rent_p02 END, y.rent_p02)
       AND d.rent_price <= COALESCE(CASE WHEN c.n_obs >= 200 THEN c.rent_p98 END, y.rent_p98) THEN 1 ELSE 0
    END AS keep_rent_trim02,
    CASE
    ...
    CASE
      WHEN d.sqft IS NULL OR d.sqft <= 0 THEN 1
      WHEN d.sqft >= COALESCE(CASE WHEN c.n_obs >= 200 THEN c.sqft_p02 END, y.sqft_p02)
       AND d.sqft <= COALESCE(CASE WHEN c.n_obs >= 200 THEN c.sqft_p98 END, y.sqft_p98) THEN 1 ELSE 0
    END AS keep_sqft_trim02,
    CASE
      WHEN d.sqft IS NULL OR d.sqft <= 0 THEN 1
  ...
  counts AS (
    SELECT
      *,
      ...
      CASE
        WHEN key_source = 'unit_id' THEN COUNT(*) OVER (PARTITION BY unit_id, year)
        ELSE NULL
      END AS unit_year_days,
...
dbExecute(
  con,
  "
  CREATE OR REPLACE TEMP TABLE native_posting_final AS
  WITH base AS (
    SELECT
      month_start,
      file_date,
  ...
  SELECT
    ...
    COUNT(*) OVER (PARTITION BY b.unit_id, b.year) AS unit_year_postings,
    CASE
      WHEN b.rent_price >= COALESCE(CASE WHEN c.n_obs >= 200 THEN c.rent_p01 END, y.rent_p01)
       AND b.rent_price <= COALESCE(CASE WHEN c.n_obs >= 200 THEN c.rent_p99 END, y.rent_p99) THEN 1 ELSE 0
    END AS keep_rent_trim01,
    CASE
      WHEN b.sqft IS NULL OR b.sqft <= 0 THEN 1
...
message("Computing weighted and trimmed series variants...")
series_variant_definitions <- data.table(
  variant_id = c(
    "raw_row_all",
    "clean_day_fullkey_equal",
    "clean_day_fullkey_invyear",
    "clean_day_fullkey_invyear_trim01_sqft01",
    "clean_day_fullkey_invyear_trim02_sqft02",
    "clean_day_fullkey_invyear_trim05_sqft05",
    "clean_day_cyclefirst_trim01_sqft01",
    "clean_day_native_invyear_trim01_sqft01",
    "native_posting_invyear_trim01_sqft01",
    "clean_day_fullkey_invyear_trim01_sqft01_btstd"
  ),
  variant_label = c(
    "Raw Positive Rows",
    "Clean Key-Day Equal Weight",
    "Clean Key-Day Inverse Key-Year Weight",
    "Clean Key-Day Inverse Weight + 1% Rent/SQFT Trim",
    "Clean Key-Day Inverse Weight + 2% Rent/SQFT Trim",
    "Clean Key-Day Inverse Weight + 5% Rent/SQFT Trim",
    "First Key-Day in 90-Day Cycle + 1% Trim",
    "Native UNIT_ID Key-Day Inverse Weight + 1% Trim",
    "Native UNIT_ID Posting Inverse Weight + 1% Trim",
    "Building-Type Standardized Clean Key-Day + 1% Trim"
  ),
  description = c(
    "All positive-rent rows with no deduplication.",
    "One observation per analysis_key x day after dropping same-day multi-rent conflicts.",
    "Clean analysis_key x day observations weighted by inverse count of clean key-days in the same year.",
    "Inverse-weighted clean key-days with 1% rent and SQFT trims within bedroom-year cells.",
    "Inverse-weighted clean key-days with 2% rent and SQFT trims within bedroom-year cells.",
    "Inverse-weighted clean key-days with 5% rent and SQFT trims within bedroom-year cells.",
    "First clean listing day in each 90-day rental cycle, after 1% rent and SQFT trims.",
    "Native UNIT_ID clean key-days only, inverse-weighted within unit-year after 1% trims.",
    "Native UNIT_ID raw postings, inverse-weighted within unit-year after 1% trims.",
    "Variant 4 reweighted to a fixed 2024 building-type mix."
  ),
  uses_fingerprint = c(FALSE, TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, FALSE, FALSE, TRUE),
  uses_native_unit = c(FALSE, TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, TRUE),
  same_day_conflicts_dropped = c(FALSE, TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, FALSE, TRUE),
  inverse_year_weight = c(FALSE, FALSE, TRUE, TRUE, TRUE, TRUE, FALSE, TRUE, TRUE, TRUE),
  rent_trim_pct = c(NA_real_, NA_real_, NA_real_, 0.01, 0.02, 0.05, 0.01, 0.01, 0.01, 0.01),
  sqft_trim_pct = c(NA_real_, NA_real_, NA_real_, 0.01, 0.02, 0.05, 0.01, 0.01, 0.01, 0.01),
  first_cycle_only = c(FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, TRUE, FALSE, FALSE, FALSE),
  building_type_standardized = c(FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, TRUE)
)

monthly_rent_series_variants <- collect_query(
  "
  SELECT
    'clean_day_fullkey_equal' AS variant_id,
    month_start,
    AVG(rent_price) AS series_value,
    QUANTILE_CONT(rent_price, 0.50) AS series_median_rent,
    COUNT(*) AS n_obs,
  ...
  SELECT
    'clean_day_fullkey_invyear' AS variant_id,
    month_start,
    SUM((1.0 / key_year_days) * rent_price) / SUM(1.0 / key_year_days) AS series_value,
    QUANTILE_CONT(rent_price, 0.50) AS series_median_rent,
    COUNT(*) AS n_obs,
    COUNT(DISTINCT analysis_key) AS n_distinct_keys,
    AVG(CASE WHEN key_source = 'unit_id' THEN 1.0 ELSE 0.0 END) AS share_unit_id_obs,
    AVG(CASE WHEN key_source = 'fingerprint' THEN 1.0 ELSE 0.0 END) AS share_fingerprint_obs,
    SUM(1.0 / key_year_days) AS weight_sum
  FROM series_day_final
  GROUP BY 1, 2
  ...
  SELECT
    'clean_day_fullkey_invyear_trim01_sqft01' AS variant_id,
    month_start,
    SUM((1.0 / key_year_days) * rent_price) / SUM(1.0 / key_year_days) AS series_value,
    QUANTILE_CONT(rent_price, 0.50) AS series_median_rent,
    COUNT(*) AS n_obs,
    ...
    SUM(1.0 / key_year_days) AS weight_sum
  FROM series_day_final
  WHERE keep_rent_trim01 = 1
    AND keep_sqft_trim01 = 1
  GROUP BY 1, 2

  UNION ALL

  SELECT
    'clean_day_fullkey_invyear_trim02_sqft02' AS variant_id,
    month_start,
    SUM((1.0 / key_year_days) * rent_price) / SUM(1.0 / key_year_days) AS series_value,
    QUANTILE_CONT(rent_price, 0.50) AS series_median_rent,
    COUNT(*) AS n_obs,
    ...
    SUM(1.0 / key_year_days) AS weight_sum
  FROM series_day_final
  WHERE keep_rent_trim02 = 1
    AND keep_sqft_trim02 = 1
  GROUP BY 1, 2

  UNION ALL

  SELECT
    'clean_day_fullkey_invyear_trim05_sqft05' AS variant_id,
    month_start,
    SUM((1.0 / key_year_days) * rent_price) / SUM(1.0 / key_year_days) AS series_value,
    QUANTILE_CONT(rent_price, 0.50) AS series_median_rent,
    COUNT(*) AS n_obs,
    ...
    SUM(1.0 / key_year_days) AS weight_sum
  FROM series_day_final
  WHERE keep_rent_trim05 = 1
    AND keep_sqft_trim05 = 1
  GROUP BY 1, 2

  UNION ALL

  SELECT
    'clean_day_cyclefirst_trim01_sqft01' AS variant_id,
    month_start,
    AVG(rent_price) AS series_value,
    QUANTILE_CONT(rent_price, 0.50) AS series_median_rent,
    COUNT(*) AS n_obs,
  ...
  WHERE keep_rent_trim01 = 1
    AND keep_sqft_trim01 = 1
    AND cycle_row_number = 1
  GROUP BY 1, 2

  UNION ALL

  SELECT
    'clean_day_native_invyear_trim01_sqft01' AS variant_id,
    month_start,
    SUM((1.0 / unit_year_days) * rent_price) / SUM(1.0 / unit_year_days) AS series_value,
    QUANTILE_CONT(rent_price, 0.50) AS series_median_rent,
    COUNT(*) AS n_obs,
    COUNT(DISTINCT unit_id) AS n_distinct_keys,
    1.0 AS share_unit_id_obs,
    0.0 AS share_fingerprint_obs,
    SUM(1.0 / unit_year_days) AS weight_sum
  FROM series_day_final
  WHERE key_source = 'unit_id'
  ...
  SELECT
    'native_posting_invyear_trim01_sqft01' AS variant_id,
    month_start,
    SUM((1.0 / unit_year_postings) * rent_price) / SUM(1.0 / unit_year_postings) AS series_value,
    QUANTILE_CONT(rent_price, 0.50) AS series_median_rent,
    COUNT(*) AS n_obs,
    COUNT(DISTINCT unit_id) AS n_distinct_keys,
    1.0 AS share_unit_id_obs,
    0.0 AS share_fingerprint_obs,
    SUM(1.0 / unit_year_postings) AS weight_sum
  FROM native_posting_final
  WHERE keep_rent_trim01 = 1
    AND keep_sqft_trim01 = 1
  GROUP BY 1, 2
  "
)
monthly_rent_series_variants[, month_start := as.Date(month_start)]
message("Variant series aggregation complete.")

raw_variant <- monthly_rent_series[, .(
  variant_id = "raw_row_all",
  month_start,
  series_value = raw_mean_rent,
  series_median_rent = raw_median_rent,
  n_obs = raw_positive_rent_rows,
  n_distinct_keys = NA_real_,
  share_unit_id_obs = NA_real_,
  share_fingerprint_obs = NA_real_,
  weight_sum = raw_positive_rent_rows
)]

type_month_variant <- collect_query(
  "
  SELECT
    month_start,
    COALESCE(building_type_clean, 'missing') AS building_type_clean,
    SUM((1.0 / key_year_days) * rent_price) / SUM(1.0 / key_year_days) AS type_weighted_mean_rent,
    COUNT(*) AS n_obs,
    SUM(1.0 / key_year_days) AS weight_sum
  FROM series_day_final
  WHERE keep_rent_trim01 = 1
...
type_month_variant[, month_start := as.Date(month_start)]
type_ref_weights <- type_month_variant[month_start >= as.Date("2024-01-01") & month_start <= as.Date("2024-12-01"),
  .(ref_weight = sum(weight_sum, na.rm = TRUE)),
  by = building_type_clean
]
if (sum(type_ref_weights$ref_weight, na.rm = TRUE) > 0) {
  type_ref_weights[, ref_weight := ref_weight / sum(ref_weight, na.rm = TRUE)]
}

bt_std_variant <- merge(type_month_variant, type_ref_weights, by = "building_type_clean", all.x = TRUE, sort = TRUE)
bt_std_variant <- bt_std_variant[!is.na(ref_weight)]
bt_std_variant <- bt_std_variant[, .(
  variant_id = "clean_day_fullkey_invyear_trim01_sqft01_btstd",
  series_value = if (sum(ref_weight[is.finite(type_weighted_mean_rent)], na.rm = TRUE) >= 0.95) {
    sum(ref_weight * type_weighted_mean_rent, na.rm = TRUE) / sum(ref_weight[is.finite(type_weighted_mean_rent)], na.rm = TRUE)
  } else {
    NA_real_
  }
), by = month_start]
bt_std_variant <- merge(
  bt_std_variant,
  monthly_rent_series_variants[variant_id == "clean_day_fullkey_invyear_trim01_sqft01", .(
    month_start,
    series_median_rent,
    n_obs,
    n_distinct_keys,
...
monthly_rent_series_variants <- rbindlist(
  list(raw_variant, monthly_rent_series_variants, bt_std_variant),
  use.names = TRUE,
  fill = TRUE
)
variant_month_grid <- CJ(
  variant_id = series_variant_definitions$variant_id,
  month_start = monthly_skeleton$month_start,
  unique = TRUE
)
monthly_rent_series_variants <- merge(
  variant_month_grid,
  monthly_rent_series_variants,
  by = c("variant_id", "month_start"),
  all.x = TRUE,
  sort = TRUE
)
monthly_rent_series_variants <- merge(
  monthly_rent_series_variants,
  series_variant_definitions[, .(variant_id, variant_label)],
  by = "variant_id",
  all.x = TRUE,
  sort = TRUE
)
...
clean_day_support <- monthly_rent_series_variants[variant_id == "clean_day_fullkey_equal", .(
  month_start,
  clean_day_n_obs = n_obs
)]
monthly_rent_series_variants <- merge(monthly_rent_series_variants, clean_day_support, by = "month_start", all.x = TRUE, sort = TRUE)
monthly_rent_series_variants[, retention_share_vs_clean_day := fifelse(
  variant_id == "raw_row_all" | !is.finite(clean_day_n_obs) | clean_day_n_obs <= 0,
  NA_real_,
  n_obs / clean_day_n_obs
)]
monthly_rent_series_variants[, support_status := vapply(n_obs, classify_support_status, character(1))]

variant_benchmark_base <- unique(monthly_rent_series[, .(
  month_start,
  zillow_city_zori,
  zillow_city_zori_yoy_pct,
  zillow_metro_zori,
...
series_variant_benchmark_comparison <- rbindlist(lapply(unique(monthly_rent_series_variants$variant_id), function(v) {
  dt <- monthly_rent_series_variants[variant_id == v]
  dt <- merge(monthly_skeleton[, .(month_start)], dt, by = "month_start", all.x = TRUE, sort = TRUE)
  dt <- merge(dt, variant_benchmark_base, by = "month_start", all.x = TRUE, sort = TRUE)
  dt[, series_sm3 := zoo::rollapplyr(series_value, 3, mean, fill = NA_real_, partial = FALSE)]
  dt[, series_yoy_pct := 100 * (series_sm3 / shift(series_sm3, 12) - 1)]

  zillow_roll_v <- rolling_compare_metrics(
    dt = dt,
    x_col = "series_yoy_pct",
    y_col = "zillow_city_zori_yoy_pct",
    lags = 0L,
    window_n = 24L,
    min_obs = 12L
  ...
  setnames(
    zillow_roll_v,
    c("n_pairs", "best_corr", "best_lag", "median_abs_gap_pp"),
    c("zillow_pairs_roll24", "zillow_corr_roll24", "zillow_best_lag_roll24", "zillow_median_abs_gap_pp_roll24")
  )

  fred_roll_v <- rolling_compare_metrics(
    dt = dt,
    x_col = "series_yoy_pct",
    y_col = "fred_chi_rent_cpi_yoy_pct",
    lags = 0L:6L,
  ...
  setnames(
    fred_roll_v,
    c("n_pairs", "best_corr", "best_lag", "median_abs_gap_pp"),
    c("fred_pairs_roll24", "fred_best_corr_roll24", "fred_best_lag_roll24", "fred_median_abs_gap_pp_roll24")
  )

  dt <- merge(dt, zillow_roll_v, by = "month_start", all.x = TRUE, sort = TRUE)
  dt <- merge(dt, fred_roll_v, by = "month_start", all.x = TRUE, sort = TRUE)
  dt[, benchmark_status := vapply(seq_len(.N), function(i) {
    classify_benchmark_status(
      corr_zillow = zillow_corr_roll24[i],
      gap_zillow_pp = zillow_median_abs_gap_pp_roll24[i],
      corr_fred = fred_best_corr_roll24[i]
  ...
  dt[is.na(benchmark_status), benchmark_status := "RED"]
  dt[, overall_status := vapply(seq_len(.N), function(i) {
    pick_worst_status(c(support_status[i], benchmark_status[i]))
  }, character(1))]
  dt[, variant_id := v]
  dt
}), fill = TRUE)
message("Variant benchmark comparison complete.")

series_variant_period_lookup <- data.table(
  period = c("2014-2016", "2017-2020", "2021-2025", "2023-2025", "2024-2025", "2014-2025"),
  start_month = as.Date(c("2014-01-01", "2017-01-01", "2021-01-01", "2023-01-01", "2024-01-01", "2014-01-01")),
  end_month = as.Date(c("2016-12-01", "2020-12-01", "2025-12-01", "2025-12-01", "2025-12-01", "2025-12-01"))
)

series_variant_period_summary <- rbindlist(lapply(unique(series_variant_benchmark_comparison$variant_id), function(v) {
  variant_dt <- series_variant_benchmark_comparison[variant_id == v]
  rbindlist(lapply(seq_len(nrow(series_variant_period_lookup)), function(i) {
    p <- series_variant_period_lookup[i]
    period_dt <- variant_dt[month_start >= p$start_month & month_start <= p$end_month]
    zillow_metrics <- compute_period_metrics(period_dt, "series_yoy_pct", "zillow_city_zori_yoy_pct", lags = 0L, min_obs = 12L)
    fred_metrics <- compute_period_metrics(period_dt, "series_yoy_pct", "fred_chi_rent_cpi_yoy_pct", lags = 0L:6L, min_obs = 12L)
    ...
    if (is.na(bench_status)) {
      bench_status <- "RED"
    }
    support_status <- classify_support_status(median(period_dt$n_obs, na.rm = TRUE))
    verdict <- pick_worst_status(c(bench_status, support_status))
    ...
    data.table(
      ...
      n_months = nrow(period_dt),
      n_green = sum(period_dt$overall_status == "GREEN", na.rm = TRUE),
      n_yellow = sum(period_dt$overall_status == "YELLOW", na.rm = TRUE),
      n_red = sum(period_dt$overall_status == "RED", na.rm = TRUE),
      share_nonred = mean(period_dt$overall_status != "RED", na.rm = TRUE),
      longest_nonred_run = longest_run(period_dt$overall_status != "RED"),
      median_n_obs = median(period_dt$n_obs, na.rm = TRUE),
      median_share_unit_id_obs = median(period_dt$share_unit_id_obs, na.rm = TRUE),
...
series_variant_period_summary <- merge(
  series_variant_period_summary,
  series_variant_definitions[, .(variant_id, variant_label)],
  by = "variant_id",
  all.x = TRUE,
  sort = TRUE
)
series_variant_period_summary[, verdict_rank := status_rank[verdict]]
setorder(series_variant_period_summary, period, verdict_rank, -share_nonred, -zillow_corr)
series_variant_period_summary[, verdict_rank := NULL]

series_variant_window_candidates <- rbindlist(lapply(unique(series_variant_benchmark_comparison$variant_id), function(v) {
  variant_dt <- series_variant_benchmark_comparison[variant_id == v][order(month_start)]
  window_dt <- recommended_status_window(variant_dt$month_start, variant_dt$overall_status)
  if (!is.finite(window_dt$n_months[1]) || window_dt$n_months[1] <= 0) {
    return(data.table(
      variant_id = v,
      window_start = as.Date(NA),
      window_end = as.Date(NA),
      n_months = 0L,
      share_green = NA_real_,
      median_n_obs = NA_real_,
      median_share_unit_id_obs = NA_real_
    ))
  }
  in_window <- variant_dt$month_start >= window_dt$window_start[1] & variant_dt$month_start <= window_dt$window_end[1]
  data.table(
    variant_id = v,
    window_start = window_dt$window_start[1],
    window_end = window_dt$window_end[1],
    n_months = window_dt$n_months[1],
    share_green = window_dt$share_green[1],
    median_n_obs = median(variant_dt$n_obs[in_window], na.rm = TRUE),
    median_share_unit_id_obs = median(variant_dt$share_unit_id_obs[in_window], na.rm = TRUE)
  )
}), fill = TRUE)
series_variant_window_candidates <- merge(
  series_variant_window_candidates,
  series_variant_definitions[, .(variant_id, variant_label)],
  by = "variant_id",
  all.x = TRUE,
  sort = TRUE
)
setorder(series_variant_window_candidates, -n_months, -share_green, -median_n_obs)
...
assert_unique(monthly_rent_series_variants, c("variant_id", "month_start"), "monthly_rent_series_variants")
assert_unique(external_benchmark_comparison, "month_start", "external_benchmark_comparison")
assert_unique(series_variant_benchmark_comparison, c("variant_id", "month_start"), "series_variant_benchmark_comparison")
...
assert_unique(series_variant_period_summary, c("variant_id", "period"), "series_variant_period_summary")
assert_unique(series_variant_window_candidates, "variant_id", "series_variant_window_candidates")
...
write_csv_dt(series_variant_definitions, file.path(out_dir, "series_variant_definitions.csv"))
write_csv_dt(monthly_rent_series_variants, file.path(out_dir, "monthly_rent_series_variants.csv"))
write_csv_dt(series_variant_benchmark_comparison, file.path(out_dir, "series_variant_benchmark_comparison.csv"))
write_csv_dt(series_variant_period_summary, file.path(out_dir, "series_variant_period_summary.csv"))
write_csv_dt(series_variant_window_candidates, file.path(out_dir, "series_variant_window_candidates.csv"))
...
variant_ranking <- copy(series_variant_period_summary[period == "2014-2025"])
variant_ranking[, verdict_rank := status_rank[verdict]]
setorder(variant_ranking, verdict_rank, -share_nonred, -zillow_corr, -median_n_obs)
top_variant_ids <- variant_ranking$variant_id[seq_len(min(4L, nrow(variant_ranking)))]
top_variant_labels <- unique(series_variant_definitions[variant_id %in% top_variant_ids, .(variant_id, variant_label)])

variant_overlay_dt <- series_variant_benchmark_comparison[variant_id %in% top_variant_ids, .(
  variant_id,
  month_start,
  index_value = series_value
)]
variant_overlay_dt <- merge(variant_overlay_dt, top_variant_labels, by = "variant_id", all.x = TRUE, sort = TRUE)
variant_overlay_dt <- rbindlist(list(
  variant_overlay_dt,
  data.table(variant_id = "zillow_city", variant_label = "Zillow City ZORI", month_start = monthly_rent_series$month_start, index_value = monthly_rent_series$zillow_city_zori),
  data.table(variant_id = "fred_rent_cpi", variant_label = "FRED Rent CPI", month_start = monthly_rent_series$month_start, index_value = monthly_rent_series$fred_chi_rent_cpi)
), fill = TRUE)
variant_overlay_dt[, index_value := {
  base_idx <- which(is.finite(index_value))[1]
  if (length(base_idx) == 1L && is.finite(index_value[base_idx]) && index_value[base_idx] != 0) {
    100 * index_value / index_value[base_idx]
  } else {
    rep(NA_real_, .N)
  }
}, by = variant_label]

p_variant_overlay <- ggplot(variant_overlay_dt, aes(month_start, index_value, color = variant_label)) +
  geom_line(linewidth = 0.8) +
  scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
  labs(x = NULL, y = "Index (first observed = 100)", color = NULL, title = "Top Cleaned RentHub Variants Versus Chicago Benchmarks") +
  build_plot_theme()
ggsave(file.path(out_dir, "fig_series_variant_overlay.pdf"), p_variant_overlay, width = 13, height = 4.8, bg = "white")

best_variant_id <- top_variant_ids[1]
best_variant_label <- top_variant_labels[variant_id == best_variant_id, variant_label][1]
best_variant_period <- series_variant_period_summary[period == "2014-2025" & variant_id == best_variant_id]
best_variant_fred_lag <- if (nrow(best_variant_period) > 0 && is.finite(best_variant_period$fred_best_lag[1])) best_variant_period$fred_best_lag[1] else 0L

variant_yoy_dt <- series_variant_benchmark_comparison[variant_id %in% top_variant_ids[seq_len(min(3L, length(top_variant_ids)))], .(
  variant_id,
  month_start,
  yoy_pct = series_yoy_pct
)]
variant_yoy_dt <- merge(variant_yoy_dt, top_variant_labels, by = "variant_id", all.x = TRUE, sort = TRUE)
variant_yoy_dt <- rbindlist(list(
  variant_yoy_dt[, .(variant_label, month_start, yoy_pct)],
  data.table(variant_label = "Zillow City ZORI", month_start = monthly_rent_series$month_start, yoy_pct = monthly_rent_series$zillow_city_zori_yoy_pct),
  data.table(
    variant_label = sprintf("FRED Rent CPI YoY (lead %d)", best_variant_fred_lag),
    month_start = monthly_rent_series$month_start,
    yoy_pct = data.table::shift(monthly_rent_series$fred_chi_rent_cpi_yoy_pct, n = best_variant_fred_lag, type = "lead")
  )
), fill = TRUE)

p_variant_yoy <- ggplot(variant_yoy_dt, aes(month_start, yoy_pct, color = variant_label)) +
  geom_line(linewidth = 0.8) +
  scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
  labs(
    x = NULL,
    y = "YoY Growth (%)",
    color = NULL,
    title = sprintf("Top Variant YoY Comparison: %s and Benchmarks", best_variant_label)
  ) +
  build_plot_theme()
ggsave(file.path(out_dir, "fig_series_variant_yoy.pdf"), p_variant_yoy, width = 13, height = 4.8, bg = "white")
