...
if (market == "rent") {
  ...
  rent <- read_parquet("../input/rent_with_ward_distances_full_drop_income.parquet") %>%
    as_tibble()
  ...
  sales <- read_parquet("../input/sales_with_hedonics_amenities_drop_income.parquet") %>%
    as_tibble()
...
for (cut_i in cutoffs_ft) {
  ...
  result_rows[[length(result_rows) + 1L]] <- tibble(
    ...
    clusters = n_distinct(d[[cluster_var]])
  )
  facet_label <- sprintf(
    "%s\nJump = %.3f%s (SE %.3f)",
    cutoff_text,
