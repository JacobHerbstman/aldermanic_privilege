...
if (length(cli_args) >= 6) {
  ...
  if (!exists("input") || !exists("bw_ft") || !exists("use_controls") || !exists("bins_per_side") || !exists("min_strictness_diff_pctile") || !exists("output_pdf")) {
    stop("FATAL: Script requires 6 args: <input> <bw_ft> <use_controls> <bins_per_side> <min_strictness_diff_pctile> <output_pdf>", call. = FALSE)
  }
}
...
ggplot() +
  ...
  labs(title = "Sale Prices by Side of Ward Boundary (FE-Adjusted)",
       subtitle = sprintf("bw=%d ft | controls=%s%s", bw_ft, use_controls, pctile_label),
