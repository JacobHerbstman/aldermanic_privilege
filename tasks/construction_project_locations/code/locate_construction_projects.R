...
# Build preferred project geography
...
replacements <- readr::read_csv("../input/preferred_residential_project_candidates.csv",
...
unchanged <- readr::read_csv("../input/preferred_residential_project_candidates.csv",
...
permit_points <- sf::st_read("../input/building_permits_for_verification.gpkg",
...
# Build preferred boundary scope
...
project_scope <- project_coverage %>%
  ...
  mutate(
    geography_status = case_when(
      ...
      location_source == "reviewed_exact_parcel_point" ~ "reviewed_parcel_location",
...
if (any(!project_scope$complete_project_geometry & !is.na(project_scope$ward) &
    !project_scope$location_source %in% c("reviewed_completed_permit_point", "reviewed_exact_parcel_point"))) {
