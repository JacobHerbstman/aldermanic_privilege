...
plot_title <- "Discontinuity in Development Density at Ward Boundary (Donut RD)"
# ================================================================================
...
plot2 <- ggplot() +
    # Add shaded donut region to show excluded area
    annotate("rect",
        xmin = -donut, xmax = donut,
        ymin = -Inf, ymax = Inf,
...
if (!exists("output_filename_rdplot")) {
    ...
    output_filename_rdplot <- sprintf(
        "../output/rd_plot%s_%s_bw%d_%s_donut%d.pdf",
        log_suffix, yvar, bw, kernel, donut
    )
}
