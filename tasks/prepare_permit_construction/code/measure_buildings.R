...
residential <- cards |>
  ...
  group_by(permit_id, record_id) |> summarise(
    ...
    single_family = all(class %in% single_family_assessor_classes), .groups = "drop") |>
...
condominiums <- condominiums |> filter(!predates_permit) |>
  ...
  transmute(permit_id, record_id, permit_pin, first_year, year_built, classes = "299", units, building_sqft, land_sqft,
    older_building = FALSE, single_family = FALSE, source = "condominium")
...
records <- bind_rows(condominiums, residential, commercial) |>
  ...
  left_join(permits |> select(permit_id, issue_date, issue_year, address), by = "permit_id", relationship = "many-to-one")
...
buildings <- records |> left_join(groups |> select(permit_id, building_id), by = "permit_id", relationship = "many-to-one") |>
  ...
  group_by(building_id) |> summarise(source = first(source),
    ...
    older_building = any(older_building), single_family = all(single_family), .groups = "drop") |>
...
# One row per measured building (identified by its first permit) and per permit that reached no building.
buildings <- permits |>
  ...
  mutate(
    ...
    # Single-family permits must match the Assessor exactly; multifamily counts may differ by revisions.
    units_agree = if_else(coalesce(permit_units, 0L) <= 1L, dwelling_units == permit_units,
      abs(dwelling_units - permit_units) <= pmax(1, unit_tolerance * permit_units)),
    flags = if_else(status != "measured" | manual_accept %in% "TRUE", "", str_c(
      if_else(!is.na(permit_units) & !units_agree, "units_disagree;", "", ""),
