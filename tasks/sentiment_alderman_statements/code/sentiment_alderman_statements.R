...
strictness_scores <- read_csv("../input/aldermen_strictness_scores.csv")
...
EARLY_WIN <- 10     # only look at first EARLY_WIN tokens for early cue
...
custom_unigrams_neg <- c(
  ...
  "strip","stripped","dilution","barrier","trump","discriminatory","impacted", "voices", 
  "citywide", "affordable", "end", "transparent", "machine", "limits", "tradition"
)
...
polarity_dt <- data.table(
  ...
  y = c(rep( 1, length(custom_unigrams_pos)),
...
early_cue <- function(txt) {
  ...
  for (i in seq_len(m)) {
    ...
    if (one %in% hot_tokens || two %in% hot_bigrams)  return(+exp(-K_DECAY*(i-1)))
    if (one %in% cold_tokens || two %in% cold_bigrams) return(-exp(-K_DECAY*(i-1)))
  }
  0
}
...
correlation_df <- scores %>% inner_join(strictness_scores, by = "alderman") %>% 
  relocate(alderman, strictness_sd, hot_cold_01)
...
# Label string
lbl <- sprintf("Correlation = %.2f",
               b)

xr <- range(correlation_df$strictness_sd,  na.rm = TRUE)
yr <- range(correlation_df$hot_cold_01,    na.rm = TRUE)
x_pos <- xr[1] + 0.02 * diff(xr)
y_pos <- yr[2] - 0.02 * diff(yr)

p <- ggplot(correlation_df, aes(x = strictness_sd, y = hot_cold_01)) +
  geom_point() +
