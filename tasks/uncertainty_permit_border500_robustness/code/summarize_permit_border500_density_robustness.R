...
variant_suffix <- if (length(args) >= 2) args[2] else "permit_border500"
variant_label <- if (length(args) >= 3) args[3] else "Drop permits within 500 ft of ward borders"
comparison_output <- if (length(args) >= 4) args[4] else file.path(output_dir, "density_score_robustness_comparison_permit_border500.csv")
comparison_tex_output <- if (length(args) >= 5) args[5] else file.path(output_dir, "density_score_robustness_comparison_permit_border500.tex")
parcel_sign_output <- if (length(args) >= 6) args[6] else file.path(output_dir, "parcel_sign_agreement_baseline_vs_permit_border500.csv")
...
names(variant_rows)[names(variant_rows) == "estimate"] <- variant_estimate_col
names(variant_rows)[names(variant_rows) == "se"] <- variant_se_col
names(variant_rows)[names(variant_rows) == "p_value"] <- variant_p_col

comparison_df <- baseline_rows %>%
  inner_join(variant_rows, by = "comparison") %>%
