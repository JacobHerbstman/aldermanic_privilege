...
alderman_data <- tribble(
  ...
  1, "Jesse Granato", "2003-01-01", "2003-05-18",
  1, "Manuel Flores", "2003-05-19", "2010-01-04",
  ...
  3, "Dorothy Tillman", "2003-01-01", "2007-04-26",
  3, "Pat Dowell", "2007-04-27", "2025-06-24",
  4, "Toni Preckwinkle", "2003-01-01", "2010-12-05",
  4, "Shirley Newsome", "2011-01-13", "2011-05-15",
  4, "William D. Burns", "2011-05-16", "2016-04-30",
  ...
  12, "Ray Frias", "2003-01-01", "2006-12-31",
  12, "George Cardenas", "2007-01-01", "2022-11-30",
  12, "Anabel Abarca", "2022-12-01", "2023-05-14",
  ...
  17, "Terry Peterson", "2003-01-01", "2007-01-31",
  17, "Latasha Thomas", "2007-02-01", "2015-05-17",
  17, "David Moore", "2015-05-18", "2025-06-24",
  18, "Thomas Murphy", "2003-01-01", "2010-12-31",
  18, "Derrick Curtis", "2011-01-01", "2025-06-24",
  ...
  24, "Michael Chandler", "2003-01-01", "2015-05-17",
  24, "Scott Waguespack", "2015-05-18", "2025-06-24",
  ...
  32, "Scott Waguespack", "2003-01-01", "2025-06-24",
  ...
  35, "Vilma Colom", "2003-01-01", "2007-05-20",
  35, "Rey Colon", "2007-05-21", "2015-05-17",
...
# Function to get alderman for a given date and ward using majority of the month rule
get_alderman <- function(current_year_month, current_ward) {
  ward_data <- alderman_data %>% filter(ward == current_ward)
  ...
  for (i in 1:nrow(ward_data)) {
    ...
    # Majority of the month rule with as.yearmon
    start_month <- if (as.numeric(format(term$start_date, "%d")) <= 15) {
      as.yearmon(term$start_date)
    } else {
      as.yearmon(term$start_date) + 1/12
    }
    
    end_month <- if (as.numeric(format(term$end_date, "%d")) >= 15) {
      as.yearmon(term$end_date)
    } else {
      as.yearmon(term$end_date) - 1/12
    }
    
    # Round to avoid floating point issues
    start_month <- round(start_month, 3)
    end_month <- round(end_month, 3)
    current_year_month_rounded <- round(current_year_month, 3)
    
    if (current_year_month_rounded >= start_month & current_year_month_rounded <= end_month) {
      alderman_name <- term$alderman
      break
    }
  }
  return(alderman_name)
}
