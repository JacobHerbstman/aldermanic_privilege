...
if (length(args) >= 6) {
  ...
  stop(
    "FATAL: Script requires 6 args: <baseline_input> <new_input> <joined_output> <summary_output> <summary_tex_output> <plot_output>",
    call. = FALSE
  )
}

variant_id <- if (length(args) >= 7) args[7] else "new_construction"
variant_label <- if (length(args) >= 8) args[8] else "New construction only"
...
p <- ggplot(joined, aes(x = baseline_score, y = .data[[score_col_name]])) +
  ...
  labs(
    title = paste0("Baseline vs. ", variant_label, " alderman stringency scores"),
    subtitle = sprintf(
