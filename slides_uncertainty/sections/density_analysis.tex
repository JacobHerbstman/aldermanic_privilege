...
\begin{frame}
    ...
    \vspace{0.3em}
    \textbf{Identifying variation}: At the same border in the same year, does the more uncertain side have less dense construction?
    
    \vspace{0.3em}
    Two specifications:
    \begin{itemize}
        \item \textbf{Visual RD}: Descriptive, visual evidence of discontinuities at ward borders
        \item \textbf{Regressions}: Regression with continuous uncertainty measure and stringent FEs
    \end{itemize}
\end{frame}
...
\begin{frame}[label=visual-rd-main]
    ...
    \scriptsize
    \textit{Notes}: FE-adjusted outcome bins with ward-pair $\times$ year FE; 500 ft bandwidth.
    
    \vfill\hfill
    \hyperlink{visual-rd-placebo}{\beamergotobutton{Pair + Year FE}}
    \hspace{0.2em}
    \hyperlink{visual-rd-functional}{\beamergotobutton{Levels}}
    \hspace{0.2em}
    \hyperlink{visual-rd-donut}{\beamergotobutton{Units}}
\end{frame}
...
\begin{frame}[label=density-reg-main]
    ...
    \small
    \textbf{Specification}: 
    $\ln(Y_i) = \beta \cdot \text{Uncertainty}_i + \alpha_{\text{border} \times \text{year} } + \varepsilon_i$\\
    Sample: Within 500 ft, multifamily buildings only. Standard errors clustered at border-pair level.
    
    \begin{center}
    \input{../tasks/border_pair_FE_regressions/output/fe_table_bw500_multifamily_pair_x_year_pruned.tex}
    \end{center}
    
    \vspace{0.1em}
    \textbf{Interpretation}: A 1 SD $\uparrow$ in regulatory uncertainty is associated with lower FAR/DUPAC
    ...
    \hspace{0.1em}
    \hyperlink{density-reg-donut}{\beamergotobutton{Donut}}
