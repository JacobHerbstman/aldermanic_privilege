...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item Fixed effects: ward-pair $\times$ year (interacted)
    \end{itemize}
    
    \vspace{0.3em}
    \textbf{Identifying variation}: Within the same ward pair in the same year, does the more stringent side have less dense new multifamily construction?
...
\begin{frame}[label=visual-rd-main]
    ...
    \begin{column}{0.48\textwidth}
        ...
        \textbf{Log(FAR)}
        \begin{figure}
            \includegraphics[width=\textwidth,height=0.55\textheight,keepaspectratio]{../tasks/spatial_rd_fe/output/rd_fe_plot_log_density_far_bw500_multifamily_pair_x_year_clust_segment.pdf}
        \end{figure}
    \end{column}
    \begin{column}{0.48\textwidth}
        \centering
        \textbf{Log(DUPAC)}
        \begin{figure}
            \includegraphics[width=\textwidth,height=0.55\textheight,keepaspectratio]{../tasks/spatial_rd_fe/output/rd_fe_plot_log_density_dupac_bw500_multifamily_pair_x_year_clust_segment.pdf}
        \end{figure}
    \end{column}
    \end{columns}
    
    \scriptsize
    \textit{Notes}: Multifamily sample, 500 ft bandwidth, ward-pair $\times$ year FE, segment-clustered SEs.
    ...
    \hspace{0.2em}
    \hyperlink{visual-rd-donut}{\beamergotobutton{Additive FE}}
\end{frame}
...
\begin{frame}[label=density-reg-main]
    \frametitle{Main Results: Interacted Border FE}
    
    \small
    \textbf{Specification}: 
    $\ln(Y_i) = \beta \cdot \text{Stringency}_i + \alpha_{\text{ward-pair} \times \text{year}} + \varepsilon_i$\\
    Sample: Within 500 ft, multifamily only. Main SEs clustered at boundary segment level.
    
    \begin{center}
    \input{../tasks/border_pair_FE_regressions/output/fe_table_bw500_multifamily_pair_x_year.tex}
    \end{center}
    
    \vspace{0.1em}
    \textbf{Interpretation}: Under interacted FE, density estimates attenuate substantially.

    \vspace{-0.4em}
    \vfill\hfill
    \hyperlink{density-reg-bw250}{\beamergotobutton{Ward-Pair Cluster}}
    \hspace{0.1em}
    \hyperlink{density-reg-bw1000}{\beamergotobutton{Additive FE}}
    \hspace{0.1em}
