...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item Main FE stack: \textbf{zoning code + boundary segment + construction year}
        \item Main inference: standard errors clustered by \textbf{boundary segment}
    \end{itemize}
    
    \vspace{0.3em}
    \textbf{Identifying variation}: Within the same boundary segment-year, does the more uncertain side have less dense construction?
    
    \vspace{0.3em}
    Two specifications:
    \begin{itemize}
        \item \textbf{Visual RD}: FE-adjusted discontinuity plots under the same FE stack
        \item \textbf{Regressions}: Border FE table with continuous uncertainty index
    \end{itemize}
\end{frame}
...
\begin{frame}[label=visual-rd-main]
    ...
    \vfill\hfill
    \hyperlink{visual-rd-placebo}{\beamergotobutton{Ward-Pair Cluster}}
    \hspace{0.2em}
    \hyperlink{visual-rd-functional}{\beamergotobutton{All Sample}}
    \hspace{0.2em}
    \hyperlink{visual-rd-donut}{\beamergotobutton{Segment + Year FE}}
\end{frame}
...
\begin{frame}[label=density-reg-main]
    ...
    \small
    \textbf{Specification}: 
    $\ln(Y_i) = \beta \cdot \text{Uncertainty}_i + \alpha_{\text{zone}} + \alpha_{\text{segment}} + \alpha_{\text{year}} + \varepsilon_i$\\
    ...
    \vspace{0.1em}
    \textbf{Interpretation}: A 1 SD $\uparrow$ in uncertainty is associated with lower multifamily FAR and DUPAC.
